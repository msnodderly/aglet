...
- Save path updates the item through
  `Workspace::update_item_with_reference_date(...)`, using local reference date.
...
- Save path also routes through
  `Workspace::update_item_with_reference_date(...)` for one canonical update path.
...
- Assign uses `Workspace::assign_item_manual(...)`.
- Unassign is validated through `Workspace::unassign_item_manual(...)` (see §38),
  then persisted.
...
All config mutations route through `Workspace::update_category(...)` so
