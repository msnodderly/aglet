<!-- GENERATED from docs/src/.htm — DO NOT EDIT. Run   MD   aglet-manual.md in docs/. -->

# Aglet Help

## Documentation

[Concepts Reference](aglet-manual.md)  
Items, categories, views, section types, automatic assignment, and the data
model. Start here to understand how aglet works.

[TUI Guide](aglet-tui.md)  
Complete guide to the terminal interface: keybindings for every mode,
interactive workflows, the view/category editors, and indicators.

[CLI Reference](aglet-cli.md)  
Every aglet command and flag: item, category, view, link, import, export,
filtering, sorting, and workflow commands.

[Download the full manual as a typeset PDF](aglet-manual.pdf) (Concepts, TUI, and CLI in one book).

## Quick Links

Concepts  
[Items](aglet-manual.md#items) ·
[Categories](aglet-manual.md#categories) ·
[Views](aglet-manual.md#views) ·
[Dependencies](aglet-manual.md#dependencies) ·
[Automatic assignment](aglet-manual.md#automatic-assignment)

TUI  
[Normal mode keys](aglet-tui.md#tui-normal-mode-keys) ·
[Category manager keys](aglet-tui.md#tui-category-manager-keys) ·
[View editor keys](aglet-tui.md#tui-view-editor-keys) ·
[Add an item](aglet-tui.md#add-an-item) ·
[Create a view](aglet-tui.md#create-a-view) ·
[Global settings](aglet-tui.md#global-settings)

CLI  
[About the CLI](aglet-cli.md#about-the-cli) ·
[Command chart](aglet-cli.md#cli-command-chart) ·
[Item commands](aglet-cli.md#cli-item-commands) ·
[Category commands](aglet-cli.md#cli-category-commands) ·
[Filtering and sorting](aglet-cli.md#cli-filtering) ·
[Workflow: claim / ready](aglet-cli.md#claim-an-item)

## About

Aglet is a free-form personal information manager. Items, categories, and views
are the three core concepts; the rest follows from them.

Source: [github.com/msnodderly/aglet](https://github.com/msnodderly/aglet)
