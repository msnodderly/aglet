...
2. Call `Aglet::preview_section_move(item_id, view, from_section, to_section)`.
...
| File | Change |
|------|--------|
| `crates/aglet-core/src/aglet.rs` | Expose `section_insert_targets`, `section_remove_targets`; add `SectionMovePreview` and `preview_section_move` |
