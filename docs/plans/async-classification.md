...
New method `process_classification_results(&mut self, workspace)`:
...
- For each result:
  ...
  - If hash matches: call `workspace.apply_classification_results()`, refresh, set status

New method `submit_background_classification(&mut self, workspace, item_id)`:
- Skip if `in_flight_classifications.contains(&item_id)`
- Call `workspace.prepare_background_classification(item_id, reference_date)`
...
- `crates/aglet-core/src/workspace.rs` — extract apply_classification_results, add prepare method
