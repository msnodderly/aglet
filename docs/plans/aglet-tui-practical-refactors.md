---
title: Aglet-TUI Practical Refactors
status: shipped
created: 2026-03-25
shipped: 2026-03-29
---

# Aglet-TUI Practical Refactors

## Summary

Refactor `aglet-tui` in small, behavior-preserving phases that reduce the size
...
- Move the large `#[cfg(test)]` module out of `crates/aglet-tui/src/lib.rs`
...
- Move `UndoState`, `UndoEntry`, and undo/redo application logic into a
  dedicated module such as `crates/aglet-tui/src/undo.rs`.
...
- Pull the derived screen-state work out of `crates/aglet-tui/src/app.rs`
...
- Introduce a small internal adapter around `Aglet` plus `Store` reads so TUI
- feature modules stop reaching into `workspace.store()` directly for common
  shared operations.
...
- Run `aglet-tui` tests after each phase.
