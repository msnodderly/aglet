...
2. **Pick task**: run open-item listing and select the SINGLE highest-priority item that is not already in progress, while respecting model complexity limits.
   ...
   **Query candidates by complexity (required):**
   ...
   ```bash
   cargo run --bin aglet -- --db /Users/mds/src/aglet-features.ag list \
     --any-category Aglet \
...
5. **Record claim metadata in issue note** (single-line update with branch + worktree):
   ...
   cargo run --bin aglet -- --db /Users/mds/src/aglet-features.ag edit <ITEM_ID> --append-note "$claim_note"
   ```

6. **Read item details**:

   ```bash
   cargo run --bin aglet -- --db /Users/mds/src/aglet-features.ag show <ITEM_ID>
   ```
...
7. **Create plan and save as note** (append concrete implementation steps):
   ...
   cargo run --bin aglet -- --db /Users/mds/src/aglet-features.ag edit <ITEM_ID> --append-note "$plan_note"
   ```
...
3. **Mark issue complete + add completion note (only after push succeeds)**:
   ...
   cargo run --bin aglet -- --db /Users/mds/src/aglet-features.ag edit <ITEM_ID> --append-note "$completion_note"
   cargo run --bin aglet -- --db /Users/mds/src/aglet-features.ag category assign <ITEM_ID> "Complete"
   ```
