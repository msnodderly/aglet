...
| S12 View Switching as Primary Navigation | partial | TUI view picker (`F8`) is implemented and usable; sequential next/previous navigation and explicit performance SLO validation are not yet documented. | `crates/agenda-tui/src/lib.rs`, `docs/test-script-tui-smoke-e2e.md` |
...
| S22 Undo After Accidental Move | deferred | Undo stack and `Ctrl-Z` recovery flow are not implemented. | `crates/agenda-core/src/undo.rs`, `spec/tasks.md` (T020-T023) |
