...
| S12 View Switching as Primary Navigation | partial | TUI view palette (`v`/`F8`) is implemented and usable, including in-app full view editing and next/previous navigation via `,`/`.`; explicit performance SLO validation is not yet documented. | `crates/agenda-tui/src/lib.rs`, `docs/test-script-tui-smoke-e2e.md` |
