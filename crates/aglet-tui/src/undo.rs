...
impl App {
    ...
    fn apply_entry(&mut self, entry: UndoEntry, aglet: &Aglet<'_>) -> TuiResult<UndoEntry> {
        let inverse = match entry {
            UndoEntry::ItemCreated { item_id } => {
                let item = aglet.store().get_item(item_id)?;
                aglet.delete_item(item_id, "undo")?;
            ...
            UndoEntry::ItemEdited {
                ...
                aglet.update_item_with_reference_date(&item, reference_date)?;
                inverse
            }
            UndoEntry::ItemDeleted { item } => {
                let item_id = item.id;
                let reference_date = jiff::Zoned::now().date();
                aglet.create_item_with_reference_date(&item, reference_date)?;
                let mut restored = aglet.store().get_item(item.id)?;
                ...
                aglet.update_item_with_reference_date(&restored, reference_date)?;
            ...
            UndoEntry::ItemDoneToggled { item_id, was_done } => {
                aglet.toggle_item_done(item_id)?;
            ...
            UndoEntry::NumericValueSet {
                ...
                let current_value = aglet.store().get_item(item_id).ok().and_then(|item| {
                    item.assignments
                        .get(&category_id)
            ...
            UndoEntry::LinkCreated {
                ...
                match kind {
                    ItemLinkKind::DependsOn => {
                        let _ = aglet.unlink_items_depends_on(item_id, other_id);
                    }
                    ItemLinkKind::Related => {
                        let _ = aglet.unlink_items_related(item_id, other_id);
                    }
                }
                self.set_item_selection_by_id(item_id);
            ...
            UndoEntry::LinkRemoved {
                ...
                match kind {
                    ItemLinkKind::DependsOn => {
                        let _ = aglet.link_items_depends_on(item_id, other_id);
                    }
                    ItemLinkKind::Related => {
                        let _ = aglet.link_items_related(item_id, other_id);
                    }
                }
                self.set_item_selection_by_id(item_id);
                UndoEntry::LinkCreated {
            ...
            UndoEntry::BatchDone { item_ids } => {
                for item_id in &item_ids {
                    let _ = aglet.toggle_item_done(*item_id);
                }
                UndoEntry::BatchDone {
    ...
    pub(crate) fn apply_undo(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let Some(entry) = self.undo.pop_undo() else {
            self.status = "Nothing to undo".to_string();
            return Ok(());
        };
        let desc = entry.description();
        let inverse = self.apply_entry(entry, aglet)?;
    ...
    pub(crate) fn apply_redo(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
