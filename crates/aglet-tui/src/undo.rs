...
use aglet_core::model::{Assignment, AssignmentSource, CategoryId, Item, ItemId, ItemLinkKind};
...
impl App {
    ...
    fn apply_entry(&mut self, entry: UndoEntry, workspace: &Workspace<'_>) -> TuiResult<UndoEntry> {
        let inverse = match entry {
            UndoEntry::ItemCreated { item_id } => {
                let item = workspace.store().get_item(item_id)?;
                workspace.delete_item(item_id, "undo")?;
            ...
            UndoEntry::ItemEdited {
                ...
                workspace.update_item_with_reference_date(&item, reference_date)?;
                inverse
            }
            UndoEntry::ItemDeleted { item } => {
                let item_id = item.id;
                let reference_date = jiff::Zoned::now().date();
                workspace.create_item_with_reference_date(&item, reference_date)?;
                let mut restored = workspace.store().get_item(item.id)?;
                ...
                workspace.update_item_with_reference_date(&restored, reference_date)?;
            ...
            UndoEntry::ItemDoneToggled { item_id, was_done } => {
                workspace.toggle_item_done(item_id)?;
            ...
            UndoEntry::CategoryAssigned {
                ...
                let _ = workspace.unassign_item_manual(item_id, category_id);
                self.set_item_selection_by_id(item_id);
            ...
            UndoEntry::NumericValueSet {
                ...
                let current_value = workspace.store().get_item(item_id).ok().and_then(|item| {
                    item.assignments
                        .get(&category_id)
            ...
            UndoEntry::LinkCreated {
                ...
                match kind {
                    ItemLinkKind::DependsOn => {
                        let _ = workspace.unlink_items_depends_on(item_id, other_id);
                    }
                    ItemLinkKind::Related => {
                        let _ = workspace.unlink_items_related(item_id, other_id);
                    }
                }
                self.set_item_selection_by_id(item_id);
            ...
            UndoEntry::LinkRemoved {
                ...
                match kind {
                    ItemLinkKind::DependsOn => {
                        let _ = workspace.link_items_depends_on(item_id, other_id);
                    }
                    ItemLinkKind::Related => {
                        let _ = workspace.link_items_related(item_id, other_id);
                    }
                }
                self.set_item_selection_by_id(item_id);
                UndoEntry::LinkCreated {
            ...
            UndoEntry::BatchDone { item_ids } => {
                for item_id in &item_ids {
                    let _ = workspace.toggle_item_done(*item_id);
                }
                UndoEntry::BatchDone {
    ...
    pub(crate) fn apply_undo(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        let Some(entry) = self.undo.pop_undo() else {
            self.status = "Nothing to undo".to_string();
            return Ok(());
        };
        let desc = entry.description();
        let inverse = self.apply_entry(entry, workspace)?;
    ...
    pub(crate) fn apply_redo(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
