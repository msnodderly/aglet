...
impl App {
    ...
    pub(crate) fn open_suggestion_review(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
    ...
    pub(crate) fn handle_suggestion_review_key(
        ...
        match code {
            ...
            KeyCode::Enter => {
                self.confirm_suggestion_review_item(aglet)?;
            }
            _ => {}
        }
        Ok(false)
    ...
    fn confirm_suggestion_review_item(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        ...
        let outcome = {
            ...
            for review in &item.suggestions {
                if review.accepted {
                    aglet.accept_classification_suggestion(review.suggestion.id)?;
                    accepted_count += 1;
                } else {
                    aglet.reject_classification_suggestion(review.suggestion.id)?;
                    rejected_count += 1;
                }
            }
