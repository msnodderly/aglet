...
impl App {
    ...
    pub(crate) fn open_suggestion_review(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
    ...
    pub(crate) fn handle_suggestion_review_key(
        ...
        match code {
            ...
            KeyCode::Enter => {
                self.confirm_suggestion_review_item(workspace)?;
            }
            _ => {}
        }
        Ok(false)
    ...
    fn confirm_suggestion_review_item(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        ...
        let outcome = {
            ...
            for review in &item.suggestions {
                if review.accepted {
                    workspace.accept_classification_suggestion(review.suggestion.id)?;
                    accepted_count += 1;
                } else {
                    workspace.reject_classification_suggestion(review.suggestion.id)?;
                    rejected_count += 1;
                }
            }
