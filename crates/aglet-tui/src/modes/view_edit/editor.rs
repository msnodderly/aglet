...
impl App {
    ...
    pub(crate) fn handle_view_edit_key(
        ...
        if self
            ...
            .unwrap_or(false)
        {
            self.handle_view_edit_discard_confirm_key(code, workspace)?;
            return Ok(false);
        }

        self.handle_view_edit_region_key(code, workspace)?;
        Ok(false)
    }
    ...
    fn handle_view_edit_discard_confirm_key(
        ...
        match code {
            KeyCode::Char('y') => {
                self.handle_view_edit_save(workspace)?;
            }
            KeyCode::Char('n') => {
    ...
    fn handle_view_edit_region_key(
        ...
        match code {
            ...
            KeyCode::Char('S') => {
                return self.handle_view_edit_save(workspace);
            }
            KeyCode::Char('p') => {
    ...
    fn handle_view_edit_save(&mut self, workspace: &Workspace<'_>) -> TuiResult<bool> {
        ...
        let save_result = if is_new_view {
            workspace.store().create_view(&draft)
        } else {
            workspace.store().update_view(&draft)
        };
        match save_result {
            Ok(()) => {
