    ...
    pub(crate) fn handle_view_delete_key(
        ...
        match code {
            KeyCode::Char('y') => {
                ...
                match workspace.store().delete_view(view.id) {
                    Ok(()) => {
            ...
            match code {
                KeyCode::Char('y') => {
                    ...
                    for item_id in &batch_delete_item_ids {
                        if let Ok(item) = workspace.store().get_item(*item_id) {
                    ...
                    for item_id in &batch_delete_item_ids {
                        match workspace.delete_item(*item_id, "user:tui") {
                            Ok(()) => deleted += 1,
                            Err(err) => {
                                failed += 1;
        ...
        match code {
            KeyCode::Char('y') => {
                if let Some(item_id) = self.selected_item_id() {
                    ...
                    workspace.delete_item(item_id, "user:tui")?;
                    self.refresh(workspace.store())?;
