use crate::*;
use aglet_core::date_rules::{parse_date_value_expr, render_date_value_expr, EvaluationContext};
use aglet_core::model::{AssignmentSource, ConditionMatchMode, DateValueExpr};
...
pub(crate) fn draft_match_label(kind: DateConditionDraftKind) -> String {
    match kind {
        DateConditionDraftKind::Compare(op) => {
            aglet_core::date_rules::render_compare_op(op).to_string()
        }
...
pub(crate) fn date_condition_draft_feedback(
    ...
    if draft_uses_range_fields(draft.kind) {
        ...
        feedback.preview = format!(
            "{} from {} through {} -> {}",
            aglet_core::date_rules::render_date_source(draft.source),
            from_preview,
            through_preview,
            category_name
        );
        ...
        feedback.preview = match draft.kind {
            DateConditionDraftKind::Compare(op) => format!(
                "{} {} {} -> {}",
                aglet_core::date_rules::render_date_source(draft.source),
                aglet_core::date_rules::render_compare_op(op),
                value_preview,
                category_name
            ),
...
impl App {
    ...
    fn prepare_category_for_workflow_role(
        ...
        let mut category = workspace.store().get_category(category_id)?;
        ...
        if auto_match_disabled {
            ...
            workspace.update_category(&category)?;
            let implicit_origin = format!("cat:{}", category.name);
            let implicit_assigned_item_ids: Vec<_> = workspace
                .store()
                .list_items()?
                .into_iter()
                .filter_map(|item| {
            ...
            if !implicit_assigned_item_ids.is_empty() {
                for item_id in implicit_assigned_item_ids {
                    workspace.store().unassign_item(item_id, category_id)?;
                }
                let refreshed_category = workspace.store().get_category(category_id)?;
                workspace.update_category(&refreshed_category)?;
            }
        }
    ...
    fn workflow_setup_cross_role_conflict_status(
        ...
        let selected_name = workspace.store().get_category(selected_category_id)?.name;
        let current_name = current_role_id
            .and_then(|category_id| workspace.store().get_category(category_id).ok())
    ...
    fn handle_category_manager_discard_confirm_key(
        ...
        match code {
            KeyCode::Char('y') => {
                self.save_category_manager_dirty_details(workspace)?;
    ...
    fn apply_category_inline_rename(
        ...
        match workspace.update_category(&category) {
            Ok(result) => {
    ...
    fn apply_category_inline_delete_confirm(
        ...
        match workspace.store().delete_category(category_id) {
            Ok(()) => {
                self.refresh(workspace.store())?;
    ...
    fn save_category_manager_details_note(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        if self.selected_category_is_reserved() {
            self.status = "Reserved category config is read-only".to_string();
        ...
        let result = workspace.update_category(&category)?;
        self.refresh(workspace.store())?;
        self.set_category_selection_by_id(category_id);
        self.reload_category_manager_details_note_from_selected();
        self.set_category_manager_focus(CategoryManagerFocus::Details);
    ...
    fn save_category_manager_details_also_match(
        &mut self,
        workspace: &Workspace<'_>,
    ) -> TuiResult<()> {
        if self.selected_category_is_reserved() {
    ...
    fn save_category_manager_dirty_details(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        if self.category_manager_details_note_dirty()
            && !self.category_manager_details_note_editing()
        {
            self.save_category_manager_details_note(workspace)?;
        }
        if self.category_manager_details_also_match_dirty()
            && !self.category_manager_details_also_match_editing()
        {
            self.save_category_manager_details_also_match(workspace)?;
        }
        Ok(())
    }
    ...
    fn handle_category_manager_details_key(
        ...
        if self.category_manager_details_inline_input().is_some() {
            match code {
                ...
                KeyCode::Enter => {
                    self.save_category_manager_numeric_inline_edit(workspace)?;
                    return Ok(true);
                }
                _ => {
        ...
        match code {
            ...
            KeyCode::Enter => match details_focus {
                CategoryManagerDetailsFocus::Exclusive => {
                    self.toggle_selected_category_exclusive(workspace)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::AutoMatch => {
                    self.toggle_selected_category_implicit(workspace)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::SemanticMatch => {
                    self.toggle_selected_category_semantic(workspace)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::MatchCategoryName => {
                    self.toggle_selected_category_match_category_name(workspace)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::Actionable => {
                    self.toggle_selected_category_actionable(workspace)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::AlsoMatch => {
                ...
                CategoryManagerDetailsFocus::Integer => {
                    self.toggle_selected_category_integer_mode(workspace)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::DecimalPlaces => {
                ...
                CategoryManagerDetailsFocus::ThousandsSeparator => {
                    self.toggle_selected_category_thousands_separator(workspace)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::Conditions => {
    ...
    fn persist_selected_category_numeric_format(
        ...
        workspace.store().update_category(&cat)?;
    ...
    fn outdent_selected_category(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        ...
        let result = workspace.move_category_to_parent(category_id, new_parent_id, insert_index)?;
        self.refresh(workspace.store())?;
        self.set_category_selection_by_id(category_id);
        self.status = format!(
    ...
    fn indent_selected_category_under_previous_sibling(
        ...
        let new_parent_label = self.category_manager_parent_label(new_parent_id);
        let result = workspace.move_category_to_parent(category_id, new_parent_id, None)?;
        self.refresh(workspace.store())?;
        self.set_category_selection_by_id(category_id);
        self.status = format!(
    ...
    fn reorder_selected_category_sibling(
        ...
        workspace.move_category_within_parent(category_id, delta.signum())?;
        self.refresh(workspace.store())?;
        self.set_category_selection_by_id(category_id);
        self.status = if delta < 0 {
    ...
    fn handle_workflow_setup_key(
        ...
        match code {
            ...
            KeyCode::Char('x') => {
                self.clear_workflow_role(workspace, self.workflow_setup_focus)?;
                return Ok(true);
            }
            KeyCode::Enter => {
    ...
    pub(crate) fn handle_workflow_role_picker_key(
        ...
        match code {
            ...
            KeyCode::Char('x') => {
                self.clear_workflow_role(workspace, picker.role_index)?;
                self.settings.workflow_role_picker = None;
                return Ok(true);
            }
            KeyCode::Enter => {
                ...
                if picker.role_index == 0 {
                    self.assign_ready_queue_role(workspace, row.id, preserved_selection)?;
                } else {
                    self.assign_claim_result_role(workspace, row.id, preserved_selection)?;
                }
                self.settings.workflow_role_picker = None;
                return Ok(true);
            }
    ...
    fn clear_workflow_role(
        ...
        let cleared_name = workspace.store().get_category(cleared_id)?.name;
        workspace.store().set_workflow_config(&workflow)?;
        self.refresh(workspace.store())?;
    ...
    fn handle_classification_mode_picker_key(
        ...
        match code {
            ...
            KeyCode::Enter => {
                ...
                self.apply_category_manager_classification_mode(workspace, mode)?;
                self.settings.classification_mode_picker_open = false;
                return Ok(true);
            }
            _ => {}
    ...
    pub(crate) fn handle_category_manager_key(
        ...
        if self.handle_category_manager_inline_action_key(code, workspace)? {
            return Ok(false);
        }
        if self.category_manager_discard_confirm() {
            self.handle_category_manager_discard_confirm_key(code, workspace)?;
            return Ok(false);
        }
        if self.settings.classification_mode_picker_open {
            self.handle_classification_mode_picker_key(code, workspace)?;
            return Ok(false);
        }
        if self.settings.workflow_role_picker.is_some() {
            self.handle_workflow_role_picker_key(code, workspace)?;
            return Ok(false);
        }
        if self.workflow_setup_open {
            self.handle_workflow_setup_key(code, workspace)?;
            return Ok(false);
        }
        if self.category_manager_condition_edit().is_some() {
            self.handle_condition_edit_key(code, workspace)?;
            return Ok(false);
        }
        if self.category_manager_action_edit().is_some() {
            self.handle_action_edit_key(code, workspace)?;
            return Ok(false);
        }
        if self.handle_category_manager_details_key(code, workspace)? {
            return Ok(false);
        }
        ...
        match code {
            ...
            KeyCode::Char('K') => {
                ...
                self.reorder_selected_category_sibling(-1, workspace)?;
            }
            KeyCode::Char('J') => {
                if self.category_manager_focus() == Some(CategoryManagerFocus::Details) {
                    return Ok(false);
            ...
            KeyCode::Char('<') => {
                ...
                if self.category_manager_structure_move_prefix() == Some('<') {
                    self.set_category_manager_structure_move_prefix(None);
                    self.outdent_selected_category(workspace)?;
                } else {
            ...
            KeyCode::Char('>') => {
                ...
                if self.category_manager_structure_move_prefix() == Some('>') {
                    self.set_category_manager_structure_move_prefix(None);
                    self.indent_selected_category_under_previous_sibling(workspace)?;
                } else {
                    self.set_category_manager_structure_move_prefix(Some('>'));
    ...
    fn assign_ready_queue_role(
        ...
        let category = workspace.store().get_category(category_id)?;
        let mut workflow = self.workflow_config.clone();
        ...
        let previous_ready_category_name = workflow
            .ready_category_id
            .and_then(|existing_id| workspace.store().get_category(existing_id).ok())
        ...
        let prep = Some(self.prepare_category_for_workflow_role(category_id, workspace)?);
    ...
    fn save_condition_from_picker(&mut self, workspace: &Workspace<'_>) -> TuiResult<bool> {
    ...
    fn toggle_condition_match_mode(&mut self, workspace: &Workspace<'_>) -> TuiResult<bool> {
        let category_id = match self.selected_category_row() {
            Some(row) => row.id,
            None => return Ok(false),
        };
    ...
    fn save_condition_from_date_editor(&mut self, workspace: &Workspace<'_>) -> TuiResult<bool> {
        ...
        let matcher = match draft.kind {
            DateConditionDraftKind::Range => aglet_core::model::DateMatcher::Range {
            ...
            DateConditionDraftKind::Compare(op) => aglet_core::model::DateMatcher::Compare {
                op,
                value: parse_date_value_expr(draft.value_input.trimmed()).map_err(TuiError::App)?,
            },
            DateConditionDraftKind::TodayAfter => aglet_core::model::DateMatcher::Range {
                from: parse_time_today_value_expr(draft.value_input.trimmed())
                    .map_err(TuiError::App)?,
                through: DateValueExpr::Today,
            },
            DateConditionDraftKind::TodayBefore => aglet_core::model::DateMatcher::Range {
                from: DateValueExpr::Today,
                through: parse_time_today_value_expr(draft.value_input.trimmed())
                    .map_err(TuiError::App)?,
            },
            DateConditionDraftKind::ThisAfternoon => aglet_core::model::DateMatcher::Range {
    ...
    fn delete_condition_at_list_index(&mut self, workspace: &Workspace<'_>) -> TuiResult<bool> {
        ...
        if edit.picker_open {
            match edit.editor_kind {
                ConditionEditorKind::ProfilePicker => {
                    self.handle_condition_picker_key(code, workspace, &edit)
                }
                ConditionEditorKind::DateEditor => {
                    self.handle_condition_date_key(code, workspace, &edit)
                }
            }
        } else {
            self.handle_condition_list_key(code, workspace, &edit)
        }
    }
    ...
    fn handle_condition_list_key(
        ...
        match code {
            ...
            KeyCode::Char('m') | KeyCode::Char('M') => {
                return self.toggle_condition_match_mode(workspace);
            }
            ...
            KeyCode::Char('x') if condition_count > 0 && edit.list_index < condition_count => {
                self.delete_condition_at_list_index(workspace)?;
            }
            _ => {}
        }
        Ok(true)
    ...
    fn handle_condition_date_key(
        ...
        match code {
            ...
            KeyCode::Enter => {
                ...
                self.save_condition_from_date_editor(workspace)?;
                return Ok(true);
            }
    ...
    fn handle_condition_picker_key(
        ...
        match code {
            ...
            KeyCode::Enter => {
                self.save_condition_from_picker(workspace)?;
            }
    ...
    fn save_action_from_picker(&mut self, workspace: &Workspace<'_>) -> TuiResult<bool> {
        ...
        let result = if let Some(idx) = action_index {
            workspace.update_category_action(category_id, idx, new_action)?
        } else {
            workspace.add_category_action(category_id, new_action)?.1
        };
        self.refresh(workspace.store())?;
        self.set_category_selection_by_id(category_id);
    ...
    fn delete_action_at_list_index(&mut self, workspace: &Workspace<'_>) -> TuiResult<bool> {
        ...
        let (_, result) = workspace.remove_category_action(category_id, list_index)?;
        self.refresh(workspace.store())?;
        self.set_category_selection_by_id(category_id);
    ...
    fn handle_action_edit_key(
        ...
        if edit.picker_open {
            self.handle_action_picker_key(code, workspace, &edit)
        } else {
            self.handle_action_list_key(code, workspace, &edit)
        }
    }
    ...
    fn handle_action_list_key(
        ...
        match code {
            ...
            KeyCode::Char('x') | KeyCode::Char('d')
                if action_count > 0 && edit.list_index < action_count =>
            {
                self.delete_action_at_list_index(workspace)?;
            }
            _ => {}
        }
        Ok(true)
    ...
    fn handle_action_picker_key(
        ...
        match code {
            ...
            KeyCode::Enter => {
                self.save_action_from_picker(workspace)?;
            }
