...
impl App {
    ...
    fn prepare_category_for_workflow_role(
        ...
        let mut category = aglet.store().get_category(category_id)?;
        ...
        if auto_match_disabled {
            ...
            aglet.update_category(&category)?;
            let implicit_origin = format!("cat:{}", category.name);
            let implicit_assigned_item_ids: Vec<_> = aglet
                .store()
                .list_items()?
                .into_iter()
                .filter_map(|item| {
            ...
            if !implicit_assigned_item_ids.is_empty() {
                for item_id in implicit_assigned_item_ids {
                    aglet.store().unassign_item(item_id, category_id)?;
                }
                let refreshed_category = aglet.store().get_category(category_id)?;
                aglet.update_category(&refreshed_category)?;
            }
        }
    ...
    fn workflow_setup_cross_role_conflict_status(
        ...
        let selected_name = aglet.store().get_category(selected_category_id)?.name;
        let current_name = current_role_id
            .and_then(|category_id| aglet.store().get_category(category_id).ok())
    ...
    fn handle_category_manager_discard_confirm_key(
        ...
        match code {
            KeyCode::Char('y') => {
                self.save_category_manager_dirty_details(aglet)?;
    ...
    fn apply_category_inline_rename(
        ...
        match aglet.update_category(&category) {
            Ok(result) => {
    ...
    fn apply_category_inline_delete_confirm(
        ...
        match aglet.store().delete_category(category_id) {
            Ok(()) => {
                self.refresh(aglet.store())?;
    ...
    fn save_category_manager_details_note(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        if self.selected_category_is_reserved() {
            self.status = "Reserved category config is read-only".to_string();
        ...
        let result = aglet.update_category(&category)?;
        self.refresh(aglet.store())?;
        self.set_category_selection_by_id(category_id);
        self.reload_category_manager_details_note_from_selected();
        self.set_category_manager_focus(CategoryManagerFocus::Details);
    ...
    fn save_category_manager_details_also_match(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        if self.selected_category_is_reserved() {
            self.status = "Reserved category config is read-only".to_string();
    ...
    fn save_category_manager_dirty_details(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
    ...
    fn handle_category_manager_details_key(
        ...
        match code {
            ...
            KeyCode::Enter => match details_focus {
                CategoryManagerDetailsFocus::Exclusive => {
                    self.toggle_selected_category_exclusive(aglet)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::AutoMatch => {
                    self.toggle_selected_category_implicit(aglet)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::SemanticMatch => {
                    self.toggle_selected_category_semantic(aglet)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::MatchCategoryName => {
                    self.toggle_selected_category_match_category_name(aglet)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::Actionable => {
                    self.toggle_selected_category_actionable(aglet)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::AlsoMatch => {
                ...
                CategoryManagerDetailsFocus::Integer => {
                    self.toggle_selected_category_integer_mode(aglet)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::DecimalPlaces => {
                ...
                CategoryManagerDetailsFocus::ThousandsSeparator => {
                    self.toggle_selected_category_thousands_separator(aglet)?;
                    self.set_category_manager_focus(CategoryManagerFocus::Tree);
                    return Ok(true);
                }
                CategoryManagerDetailsFocus::Conditions => {
    ...
    fn persist_selected_category_numeric_format(
        ...
        aglet.store().update_category(&cat)?;
    ...
    fn toggle_selected_category_integer_mode(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let current = self
    ...
    fn toggle_selected_category_thousands_separator(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let current = self
            .selected_category_numeric_format()
            .ok_or("No category")?;
        let mut next = current.clone();
    ...
    fn save_category_manager_numeric_inline_edit(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
    ...
    fn outdent_selected_category(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        ...
        let result = aglet.move_category_to_parent(category_id, new_parent_id, insert_index)?;
        self.refresh(aglet.store())?;
        self.set_category_selection_by_id(category_id);
        self.status = format!(
    ...
    fn indent_selected_category_under_previous_sibling(
        ...
        let new_parent_label = self.category_manager_parent_label(new_parent_id);
        let result = aglet.move_category_to_parent(category_id, new_parent_id, None)?;
        self.refresh(aglet.store())?;
        self.set_category_selection_by_id(category_id);
        self.status = format!(
    ...
    fn reorder_selected_category_sibling(
        ...
        aglet.move_category_within_parent(category_id, delta.signum())?;
        self.refresh(aglet.store())?;
        self.set_category_selection_by_id(category_id);
        self.status = if delta < 0 {
    ...
    fn handle_workflow_setup_key(&mut self, code: KeyCode, aglet: &Aglet<'_>) -> TuiResult<bool> {
        match code {
            ...
            KeyCode::Char('x') => {
                self.clear_workflow_role(aglet, self.workflow_setup_focus)?;
                return Ok(true);
            }
            KeyCode::Enter => {
    ...
    pub(crate) fn handle_workflow_role_picker_key(
        ...
        match code {
            ...
            KeyCode::Char('x') => {
                self.clear_workflow_role(aglet, picker.role_index)?;
                self.settings.workflow_role_picker = None;
                return Ok(true);
            }
            KeyCode::Enter => {
                ...
                if picker.role_index == 0 {
                    self.assign_ready_queue_role(aglet, row.id, preserved_selection)?;
                } else {
                    self.assign_claim_result_role(aglet, row.id, preserved_selection)?;
                }
                self.settings.workflow_role_picker = None;
                return Ok(true);
            }
    ...
    fn clear_workflow_role(&mut self, aglet: &Aglet<'_>, role_index: usize) -> TuiResult<()> {
        ...
        let cleared_name = aglet.store().get_category(cleared_id)?.name;
        aglet.store().set_workflow_config(&workflow)?;
        self.refresh(aglet.store())?;
    ...
    fn handle_classification_mode_picker_key(
        ...
        match code {
            ...
            KeyCode::Enter => {
                ...
                self.apply_category_manager_classification_mode(aglet, mode)?;
                self.settings.classification_mode_picker_open = false;
                return Ok(true);
            }
            _ => {}
    ...
    pub(crate) fn handle_category_manager_key(
        ...
        if self.handle_category_manager_inline_action_key(code, aglet)? {
            return Ok(false);
        }
        if self.category_manager_discard_confirm() {
            self.handle_category_manager_discard_confirm_key(code, aglet)?;
            return Ok(false);
        }
        if self.settings.classification_mode_picker_open {
            self.handle_classification_mode_picker_key(code, aglet)?;
            return Ok(false);
        }
        ...
        if self.workflow_setup_open {
            self.handle_workflow_setup_key(code, aglet)?;
            return Ok(false);
        }
        if self.category_manager_condition_edit().is_some() {
            self.handle_condition_edit_key(code, aglet)?;
            return Ok(false);
        }
        if self.category_manager_action_edit().is_some() {
            self.handle_action_edit_key(code, aglet)?;
            return Ok(false);
        }
        if self.handle_category_manager_details_key(code, aglet)? {
            return Ok(false);
        }
        ...
        match code {
            ...
            KeyCode::Char('K') => {
                ...
                self.reorder_selected_category_sibling(-1, aglet)?;
            }
            KeyCode::Char('J') => {
                if self.category_manager_focus() == Some(CategoryManagerFocus::Details) {
                    return Ok(false);
            ...
            KeyCode::Char('<') => {
                ...
                if self.category_manager_structure_move_prefix() == Some('<') {
                    self.set_category_manager_structure_move_prefix(None);
                    self.outdent_selected_category(aglet)?;
                } else {
            ...
            KeyCode::Char('>') => {
                ...
                if self.category_manager_structure_move_prefix() == Some('>') {
                    self.set_category_manager_structure_move_prefix(None);
                    self.indent_selected_category_under_previous_sibling(aglet)?;
                } else {
                    self.set_category_manager_structure_move_prefix(Some('>'));
    ...
    pub(crate) fn toggle_selected_category_implicit(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        if self.selected_category_is_reserved() {
            self.status = "Reserved category config is read-only".to_string();
            return Ok(());
        }
    ...
    pub(crate) fn toggle_selected_category_semantic(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        if self.selected_category_is_reserved() {
            self.status = "Reserved category config is read-only".to_string();
            return Ok(());
        }
    ...
    fn assign_ready_queue_role(
        ...
        let category = aglet.store().get_category(category_id)?;
        let mut workflow = self.workflow_config.clone();
        ...
        let previous_ready_category_name = workflow
            .ready_category_id
            .and_then(|existing_id| aglet.store().get_category(existing_id).ok())
        ...
        let prep = Some(self.prepare_category_for_workflow_role(category_id, aglet)?);
    ...
    fn save_condition_from_picker(&mut self, aglet: &Aglet<'_>) -> TuiResult<bool> {
    ...
    fn toggle_condition_match_mode(&mut self, aglet: &Aglet<'_>) -> TuiResult<bool> {
        let category_id = match self.selected_category_row() {
            Some(row) => row.id,
            None => return Ok(false),
        };
    ...
    fn save_condition_from_date_editor(&mut self, aglet: &Aglet<'_>) -> TuiResult<bool> {
    ...
    fn delete_condition_at_list_index(&mut self, aglet: &Aglet<'_>) -> TuiResult<bool> {
    ...
    fn handle_condition_edit_key(&mut self, code: KeyCode, aglet: &Aglet<'_>) -> TuiResult<bool> {
        let edit = match self.category_manager_condition_edit() {
            Some(e) => e.clone(),
            None => return Ok(false),
        };
        ...
        if edit.picker_open {
            match edit.editor_kind {
                ConditionEditorKind::ProfilePicker => {
                    self.handle_condition_picker_key(code, aglet, &edit)
                }
                ConditionEditorKind::DateEditor => {
                    self.handle_condition_date_key(code, aglet, &edit)
                }
            }
        } else {
            self.handle_condition_list_key(code, aglet, &edit)
        }
    }
    ...
    fn handle_condition_list_key(
        ...
        match code {
            ...
            KeyCode::Char('m') | KeyCode::Char('M') => {
                return self.toggle_condition_match_mode(aglet);
            }
            ...
            KeyCode::Char('x') if condition_count > 0 && edit.list_index < condition_count => {
                self.delete_condition_at_list_index(aglet)?;
            }
            _ => {}
        }
        Ok(true)
    ...
    fn handle_condition_date_key(
        ...
        match code {
            ...
            KeyCode::Enter => {
                ...
                self.save_condition_from_date_editor(aglet)?;
                return Ok(true);
            }
    ...
    fn handle_condition_picker_key(
        ...
        match code {
            ...
            KeyCode::Enter => {
                self.save_condition_from_picker(aglet)?;
            }
    ...
    fn save_action_from_picker(&mut self, aglet: &Aglet<'_>) -> TuiResult<bool> {
    ...
    fn delete_action_at_list_index(&mut self, aglet: &Aglet<'_>) -> TuiResult<bool> {
        ...
        let (_, result) = aglet.remove_category_action(category_id, list_index)?;
        self.refresh(aglet.store())?;
        self.set_category_selection_by_id(category_id);
    ...
    fn handle_action_edit_key(&mut self, code: KeyCode, aglet: &Aglet<'_>) -> TuiResult<bool> {
        let edit = match self.category_manager_action_edit() {
            Some(e) => e.clone(),
            None => return Ok(false),
        };
        ...
        if edit.picker_open {
            self.handle_action_picker_key(code, aglet, &edit)
        } else {
            self.handle_action_list_key(code, aglet, &edit)
        }
    }
    ...
    fn handle_action_list_key(
        ...
        match code {
            ...
            KeyCode::Char('x') | KeyCode::Char('d')
                if action_count > 0 && edit.list_index < action_count =>
            {
                self.delete_action_at_list_index(aglet)?;
            }
            _ => {}
        }
        Ok(true)
    ...
    fn handle_action_picker_key(
        ...
        match code {
            ...
            KeyCode::Enter => {
                self.save_action_from_picker(aglet)?;
            }
