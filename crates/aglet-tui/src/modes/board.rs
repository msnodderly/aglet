...
impl App {
    ...
    fn apply_item_assign_category_by_name(
        ...
        for action_item_id in &action_item_ids {
            let item = aglet.store().get_item(*action_item_id)?;
            if item.assignments.contains_key(&category_id) {
                already_had += 1;
                continue;
            }

            match aglet.assign_item_manual(
                *action_item_id,
                category_id,
            ...
            KeyCode::Char('y') => {
                ...
                match kind {
                    input_panel::InputPanelKind::AddItem => {
                        self.save_input_panel_add(aglet)?;
                    }
                    input_panel::InputPanelKind::EditItem => {
                        self.save_input_panel_edit(aglet)?;
                    }
                    _ => {
                        self.save_input_panel_name(aglet)?;
                    }
                }
            }
            KeyCode::Char('n') => {
    ...
    fn move_current_board_column_relative(
        ...
        self.move_current_board_column_to_index(target, aglet)
    }
    ...
    fn remove_current_board_column(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let Some(slot) = self.current_slot() else {
            self.status = "No active board slot".to_string();
            return Ok(());
        };
    ...
    pub(crate) fn handle_board_column_delete_confirm_key(
        ...
        match code {
            KeyCode::Char('y') => {
                ...
                self.mode = Mode::Normal;
                self.remove_current_board_column(aglet)?;
            }
            KeyCode::Esc => {
    ...
    fn confirm_inline_create_board_add_column(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let Some(name) = self
        ...
        aglet.create_category(&category)?;
        self.refresh_category_cache(aglet.store())?;
        self.set_board_add_column_create_confirm_name(None);
        self.insert_board_column_for_category(aglet, cat_id)?;
    ...
    fn apply_category_column_picker_selection(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        ...
        for id in to_remove {
            aglet.unassign_item_manual(item_id, id)?;
        }
        for id in to_add {
            aglet.assign_item_manual(
                item_id,
                id,
        ...
        match code {
            ...
            KeyCode::Enter => {
                if matches!(
                    ...
                    self.apply_category_column_picker_selection(aglet)?;
                    return Ok(false);
                }
                let typed = self
                    .category_column_picker_filter_text()
                ...
                if let Some(category_id) =
                    self.resolve_category_column_picker_typed_category(aglet)?
                {
                    if let Some(state) = self.category_column_picker_state_mut() {
    ...
    fn assign_selected_suggestion(&mut self, _aglet: &Aglet<'_>) -> TuiResult<()> {
    ...
    fn confirm_inline_create_category_direct_edit(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
    ...
    fn apply_category_direct_edit_draft(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        if self.direct_edit_create_confirm_open() {
    ...
    pub(crate) fn handle_normal_key(
        ...
        match code {
            ...
            KeyCode::Char('H') => {
                self.move_current_board_column_relative(-1, aglet)?;
            }
            KeyCode::Char('L') => {
                self.move_current_board_column_relative(1, aglet)?;
            }
            ...
            KeyCode::Char('m') => {
                self.toggle_current_view_section_flow(aglet)?;
            }
            KeyCode::Char('z') => {
                self.cycle_current_board_display_mode(aglet)?;
            }
            KeyCode::Char('s') | KeyCode::Char('<') => {
                self.sort_current_slot_by_active_column(None, aglet)?;
            }
            KeyCode::Char('S') | KeyCode::Char('>') => {
                self.sort_current_slot_by_active_column(Some(SlotSortDirection::Desc), aglet)?;
            }
            KeyCode::Char('F') => {
                self.cycle_column_summary_fn(aglet)?;
            }
            KeyCode::Char('b') => {
            ...
            KeyCode::Char('C') => {
                self.open_suggestion_review(aglet)?;
            }
            KeyCode::Enter => {
            ...
            KeyCode::Esc => {
                if self.has_selected_items() {
                    ...
                    self.restore_global_search_session(aglet)?;
                } else {
            ...
            KeyCode::Char('f') => {
                self.cycle_column_numeric_format(aglet)?;
            }
            KeyCode::Char('g') => {
            ...
            KeyCode::Char('a') => {
                if self.selected_item_id().is_none() {
                    ...
                    self.reload_item_assign_vetoes();
            ...
            KeyCode::Char('r') => {
                if let Some(item_id) = self.selected_item_id() {
                    if let Some(view) = self.current_view().cloned() {
                        aglet.remove_item_from_view(item_id, &view)?;
                        self.refresh(aglet.store())?;
            ...
            KeyCode::Char('d') | KeyCode::Char('D') => {
                if self.selected_count() > 1 {
                    self.batch_toggle_selected_items_done(aglet, DoneToggleOrigin::NormalMode)?;
            ...
            KeyCode::Char('=') => {
                ...
                if item_ids.is_empty() {
                    ...
                    for &item_id in &item_ids {
                        match self.submit_background_classification(aglet, item_id)? {
    ...
    pub(crate) fn apply_done_toggle_action(
        ...
        let result = aglet.toggle_item_done(item_id)?;

        let mut removed_blocker_links = 0usize;
        if !was_done {
            for blocked_id in clear_blocked_item_ids {
                aglet.unlink_items_blocks(item_id, *blocked_id)?;
        ...
        if let Some(successor_id) = result.successor_item_id {
            if let Ok(successor) = aglet.store().get_item(successor_id) {
    ...
    fn begin_done_toggle_or_confirm(
        ...
        self.apply_done_toggle_action(aglet, item_id, was_done, origin, &[])
    }
    ...
    fn batch_toggle_selected_items_done(
        ...
        self.apply_batch_done_action(aglet, &action_item_ids, false, origin)
    }
    ...
    pub(crate) fn apply_batch_done_action(
        ...
        for item_id in item_ids {
            ...
            match aglet.toggle_item_done(*item_id) {
                Ok(_) => {
                    changed += 1;
    ...
    fn cycle_column_summary_fn(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
    ...
    fn cycle_column_numeric_format(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let slot = match self.current_slot() {
            Some(s) => s,
            None => return Ok(()),
        };
        ...
        aglet.store().update_category(&updated_cat)?;
        self.refresh(aglet.store())?;
        self.status = format!("Column format: {}", describe_numeric_format(&next));
        Ok(())
    }

    fn cycle_current_board_display_mode(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let Some(mut view) = self.current_view().cloned() else {
            return Ok(());
        };
    ...
    fn toggle_current_view_section_flow(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let Some(mut view) = self.current_view().cloned() else {
            return Ok(());
        };
    ...
    fn apply_link_wizard(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        ...
        let (status, clear_selection) = match action {
            ...
            LinkWizardAction::ClearDependencies => {
                ...
                for source_item_id in &source_item_ids {
                    let prereqs = aglet.immediate_prereq_ids(*source_item_id)?;
                    let dependents = aglet.immediate_dependent_ids(*source_item_id)?;
                    total_prereqs += prereqs.len();
                    total_dependents += dependents.len();
                    for dependency_id in &prereqs {
                        aglet.unlink_items_depends_on(*source_item_id, *dependency_id)?;
                    }
                    for blocked_id in &dependents {
                        aglet.unlink_items_blocks(*source_item_id, *blocked_id)?;
                    }
                }
    ...
    pub(crate) fn handle_normal_key_event(
        ...
        if ctrl_only {
            match key.code {
                ...
                KeyCode::Char('z') => {
                    self.apply_undo(aglet)?;
                    return Ok(false);
                }
                KeyCode::Char('Z') => {
                    self.apply_redo(aglet)?;
                    return Ok(false);
                }
                _ => {}
            }
        ...
        if self.board_add_column_create_confirm_open() {
            match inline_create_confirm_key_action(code) {
                InlineCreateConfirmKeyAction::Confirm => {
                    self.confirm_inline_create_board_add_column(aglet)?;
                    return Ok(false);
                }
        ...
        match code {
            ...
            KeyCode::Enter => {
                if let Some(category_id) = self.exact_board_add_column_match_id() {
                    self.insert_board_column_for_category(aglet, category_id)?;
                } else {
                    let matches = self.get_board_add_column_suggest_matches();
    ...
    pub(crate) fn handle_category_direct_edit_key(
        ...
        if self.direct_edit_create_confirm_open() {
            match inline_create_confirm_key_action(code) {
                InlineCreateConfirmKeyAction::Confirm => {
                    self.confirm_inline_create_category_direct_edit(aglet)?;
                    return Ok(false);
                }
                InlineCreateConfirmKeyAction::Cancel => {
                    self.set_direct_edit_create_confirm_name(None);
        ...
        match code {
            ...
            KeyCode::Enter => {
                ...
                if active_text.trim().is_empty() {
                    ...
                    self.assign_selected_suggestion(aglet)?;
                } else {
    ...
    fn open_item_assign_from_edit_panel(&mut self, aglet: &Aglet<'_>) {
        ...
        match action {
            InputPanelAction::Save => {
                ...
                match kind {
                    ...
                    input_panel::InputPanelKind::CategoryCreate => {
                        self.save_input_panel_category_create(aglet)?;
                    }
                }
            }
    ...
    pub(crate) fn save_input_panel_add(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
            ...
            match Self::parse_when_datetime_input(&when_text) {
                Ok(Some((dt, parsed_rule))) => {
                    ...
                    if let Some(rule) = rule {
                        aglet.set_item_recurrence_rule(item.id, Some(rule))?;
                    }
                }
                Ok(None) => {}
                Err(e) => {
        ...
        if note.is_some() {
            let mut loaded = aglet.store().get_item(item.id)?;
            loaded.note = note;
            loaded.modified_at = Timestamp::now();
            let note_update_result = aglet.update_item_cheap(&loaded, reference_date)?;
            process_result
                .new_assignments
        ...
        let created = aglet.store().get_item(item.id)?;
        ...
        let _ = self.submit_background_classification(aglet, item.id);
        Ok(())
    }

    /// Save an InputPanel(EditItem) to the store (text, note, when, suggestion decisions).
    pub(crate) fn save_input_panel_edit(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let Some(panel) = &self.input_panel else {
            self.mode = Mode::Normal;
            return Ok(());
        };
        ...
        let mut item = aglet.store().get_item(item_id)?;
        ...
        let reference_date = jiff::Zoned::now().date();
        let process_result = aglet.update_item_cheap(&item, reference_date)?;
        ...
        if let Some(rule) = recurrence_rule {
            aglet.set_item_recurrence_rule(item_id, Some(rule))?;
        } else if !no_when_change && when_text.is_empty() {
            // Clearing the when field also clears recurrence.
            aglet.set_item_recurrence_rule(item_id, None)?;
        }
        ...
        for (suggestion, decision) in &pending_suggestions {
            match decision {
                SuggestionDecision::Accept => {
                    aglet.accept_classification_suggestion(suggestion.id)?;
                    suggestion_accepted += 1;
                }
                SuggestionDecision::Reject => {
                    aglet.reject_classification_suggestion(suggestion.id)?;
        ...
        self.status = status;
        // Submit background classification (Ollama) if enabled.
        let _ = self.submit_background_classification(aglet, item_id);
        Ok(())
    }
    ...
    fn save_input_panel_name(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let input_text = self
            .input_panel
            .as_ref()
        ...
        match self.name_input_context {
            Some(NameInputContext::ViewClone) => {
                ...
                match aglet.store().clone_view(source_id, name.clone()) {
                    Ok(_cloned) => {
                        self.refresh(aglet.store())?;
                        self.set_view_selection_by_name(&name);
                        self.input_panel = None;
            ...
            Some(NameInputContext::ViewRename) => {
                ...
                match aglet.store().update_view(&view) {
                    Ok(()) => {
                        self.refresh(aglet.store())?;
                        self.set_view_selection_by_name(&name);
                        self.input_panel = None;
            ...
            Some(NameInputContext::NumericValueEdit) => {
                ...
                match normalized.parse::<rust_decimal::Decimal>() {
                    Ok(decimal_value) => {
                        match aglet.assign_item_numeric_manual(
                            target.item_id,
                            target.category_id,
                            decimal_value,
            ...
            Some(NameInputContext::WhenDateEdit) => {
                ...
                match aglet.set_item_when_date(
                    target.item_id,
                    parsed_when,
                    ...
                    Ok(_result) => {
                        ...
                        if inline_rule.is_some() || parsed_when.is_none() {
                            let _ = aglet.set_item_recurrence_rule(target.item_id, inline_rule);
                        }
                        self.refresh(aglet.store())?;
    ...
    fn save_input_panel_category_create(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let Some(panel) = &self.input_panel else {
            self.mode = self.name_input_return_mode();
            self.name_input_context = None;
            return Ok(());
        ...
        match aglet.create_category(&category) {
            Ok(result) => {
                self.refresh(aglet.store())?;
    ...
    /// from the refresh-time cache (refresh runs after every mutation, so the
    /// cache is current at every call site).
    pub(crate) fn reload_item_assign_vetoes(&mut self) {
        self.item_assign_vetoes = self
            .selected_item_id()
            .and_then(|item_id| self.vetoes_by_item.get(&item_id).cloned())
            .unwrap_or_default();
    }
        ...
        match code {
            ...
            KeyCode::Char(' ') => {
                ...
                if batch_mode && action_item_ids.len() > 1 {
                    ...
                    let old_assignment = self
                        ...
                        .cloned();
                    match aglet.unassign_item_manual(item_id, row.id) {
                        Ok(result) => {
                    ...
                    let result = aglet.assign_item_manual(
                        item_id,
                        row.id,
                        Some("manual:tui.assign".to_string()),
                    )?;
            ...
            KeyCode::Enter => {
                ...
                if self.item_assign_dirty {
                    ...
                    self.handle_item_assign_category_key(KeyCode::Char(' '), aglet)?;
                    if self.mode == Mode::ItemAssignPicker {
                        self.close_item_assign_session(self.status.clone());
                    }
                }
    ...
    fn handle_item_assign_view_key(&mut self, code: KeyCode, aglet: &Aglet<'_>) -> TuiResult<bool> {
        ...
        match code {
                ...
                for item_id in &action_ids {
                    ...
                    let (res, did_change) = match current_placement {
                        ...
                        None => match section_idx {
                            ...
                            None => (
                                aglet.insert_item_in_unmatched(*item_id, &view).map(|_| ()),
                                true,
                            ),
                        },
                    };
            ...
            KeyCode::Char('r') => {
                ...
                for item_id in &action_ids {
                    ...
                    match aglet.remove_item_from_view(*item_id, &view) {
                        Ok(_) => changed += 1,
                        Err(err) => {
                            if first_error.is_none() {
                                first_error = Some(err.to_string());
            ...
            KeyCode::Enter => {
                if self.item_assign_dirty {
                    ...
                    self.handle_item_assign_view_key(KeyCode::Char(' '), aglet)?;
                    if self.mode == Mode::ItemAssignPicker {
                        self.close_item_assign_session(self.status.clone());
                    }
                }
...
pub(crate) fn exclusive_siblings_to_clear(
...
mod tests {
    ...
    fn input_panel_category_filter_narrows_rows_and_toggles_match() {
        ...
        app.handle_input_panel_key(KeyCode::Char('/'), &aglet)
            .expect("open filter");
        app.handle_input_panel_key(KeyCode::Char('b'), &aglet)
            .expect("type filter");
        assert_eq!(
    ...
    fn ctrl_g_sets_pending_external_edit_for_text() {
        ...
        let classifier = SubstringClassifier;
        let aglet = Aglet::new(&store, &classifier);
        app.handle_input_panel_key(KeyCode::Char('g'), &aglet)
            .expect("ctrl+g");
        assert_eq!(
    ...
    fn add_item_in_ready_section_with_no_children_assigns_ready_category() {
        ...
        assert!(
            assignments.contains_key(&aglet_cat.id),
