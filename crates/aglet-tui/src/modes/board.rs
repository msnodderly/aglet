...
use aglet_core::model::RecurrenceRule;
...
impl App {
    fn resolve_item_assign_category_enter(
        ...
        workspace.store().create_category(&category)?;
    ...
    fn apply_item_assign_category_by_name(
        ...
        let (category_id, category_name, created_new_category) =
            match self.resolve_item_assign_category_enter(workspace) {
                Ok(result) => result,
                Err(err) => {
                    self.mode = return_mode;
                    self.clear_input();
        ...
        for action_item_id in &action_item_ids {
            let item = workspace.store().get_item(*action_item_id)?;
            if item.assignments.contains_key(&category_id) {
                already_had += 1;
                continue;
            }

            match workspace.assign_item_manual(
                *action_item_id,
                category_id,
            ...
            KeyCode::Char('y') => {
                ...
                match kind {
                    input_panel::InputPanelKind::AddItem => {
                        self.save_input_panel_add(workspace)?;
                    }
                    input_panel::InputPanelKind::EditItem => {
                        self.save_input_panel_edit(workspace)?;
                    }
                    _ => {
                        self.save_input_panel_name(workspace)?;
                    }
                }
            }
            KeyCode::Char('n') => {
    ...
    fn insert_board_column_for_category(
        ...
        workspace.store().update_view(&view)?;
    ...
    fn move_current_board_column_relative(
        ...
        self.move_current_board_column_to_index(target, workspace)
    }
    ...
    fn remove_current_board_column(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        let Some(slot) = self.current_slot() else {
            self.status = "No active board slot".to_string();
            return Ok(());
        };
    ...
    pub(crate) fn handle_board_column_delete_confirm_key(
        ...
        match code {
            KeyCode::Char('y') => {
                ...
                self.mode = Mode::Normal;
                self.remove_current_board_column(workspace)?;
            }
            KeyCode::Esc => {
    ...
    fn confirm_inline_create_board_add_column(
        ...
        workspace.create_category(&category)?;
        self.refresh_category_cache(workspace.store())?;
        self.set_board_add_column_create_confirm_name(None);
        self.insert_board_column_for_category(workspace, cat_id)?;
    ...
    fn apply_category_column_picker_selection(
        ...
        for id in to_remove {
            workspace.unassign_item_manual(item_id, id)?;
        }
        for id in to_add {
            workspace.assign_item_manual(
                item_id,
                id,
        ...
        match code {
            ...
            KeyCode::Enter => {
                if matches!(
                    ...
                    self.apply_category_column_picker_selection(workspace)?;
                    return Ok(false);
                }
                let typed = self
                    .category_column_picker_filter_text()
                ...
                if let Some(category_id) =
                    self.resolve_category_column_picker_typed_category(workspace)?
                {
                    if let Some(state) = self.category_column_picker_state_mut() {
    ...
    fn assign_selected_suggestion(&mut self, _agenda: &Workspace<'_>) -> TuiResult<()> {
    ...
    fn apply_category_direct_edit_draft(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        if self.direct_edit_create_confirm_open() {
    ...
    pub(crate) fn handle_normal_key(
        ...
        if let Some(prefix) = self.normal_mode_prefix.take() {
            match (prefix, code) {
                (NormalModePrefix::G, KeyCode::Char('a')) => {
                    self.jump_to_all_items_view(workspace)?;
                    self.status = "Jumped to All Items view".to_string();
                    return Ok(false);
                }
                (NormalModePrefix::G, KeyCode::Char('/')) => {
                    self.begin_global_search_session(workspace)?;
                    return Ok(false);
                }
                (NormalModePrefix::G, KeyCode::Char('s')) => {
                    self.open_global_settings(workspace.store())?;
                    return Ok(false);
                }
        ...
        match code {
            ...
            KeyCode::Down if self.transient.key_modifiers.contains(KeyModifiers::SHIFT) => {
                self.move_selected_item_between_slots(1, workspace)?;
            }
            ...
            KeyCode::Char('H') => {
                self.move_current_board_column_relative(-1, workspace)?;
            }
            KeyCode::Char('L') => {
                self.move_current_board_column_relative(1, workspace)?;
            }
            ...
            KeyCode::Char('m') => {
                self.toggle_current_view_section_flow(workspace)?;
            }
            KeyCode::Char('z') => {
                self.cycle_current_board_display_mode(workspace)?;
            }
            KeyCode::Char('s') | KeyCode::Char('<') => {
                self.sort_current_slot_by_active_column(None, workspace)?;
            }
            KeyCode::Char('S') | KeyCode::Char('>') => {
                self.sort_current_slot_by_active_column(Some(SlotSortDirection::Desc), workspace)?;
            }
            KeyCode::Char('F') => {
                self.cycle_column_summary_fn(workspace)?;
            }
            KeyCode::Char('b') => {
            ...
            KeyCode::Char('C') => {
                self.open_suggestion_review(workspace)?;
            }
            KeyCode::Enter => {
            ...
            KeyCode::Esc => {
                if self.has_selected_items() {
                    ...
                    self.restore_global_search_session(workspace)?;
                } else {
            ...
            KeyCode::Char('f') => {
                self.cycle_column_numeric_format(workspace)?;
            }
            KeyCode::Char('g') => {
            ...
            KeyCode::Char('a') => {
                if self.selected_item_id().is_none() {
                    ...
                    self.compute_assignment_preview(workspace);
                    self.clear_input();
            ...
            KeyCode::Char('r') => {
                if let Some(item_id) = self.selected_item_id() {
                    if let Some(view) = self.current_view().cloned() {
                        workspace.remove_item_from_view(item_id, &view)?;
                        self.refresh(workspace.store())?;
            ...
            KeyCode::Char('d') | KeyCode::Char('D') => {
                if self.selected_count() > 1 {
                    self.batch_toggle_selected_items_done(workspace, DoneToggleOrigin::NormalMode)?;
            ...
            KeyCode::Char('}') => {
                self.datebook_browse(1, workspace)?;
            }
            KeyCode::Char('{') => {
                self.datebook_browse(-1, workspace)?;
            }
            KeyCode::Char('0') => {
                self.datebook_browse(0, workspace)?;
            }
            KeyCode::Char('=') => {
                ...
                if item_ids.is_empty() {
                    ...
                    for &item_id in &item_ids {
                        match self.submit_background_classification(workspace, item_id)? {
    ...
    pub(crate) fn apply_done_toggle_action(
        ...
        let result = workspace.toggle_item_done(item_id)?;

        let mut removed_blocker_links = 0usize;
        if !was_done {
            for blocked_id in clear_blocked_item_ids {
                workspace.unlink_items_blocks(item_id, *blocked_id)?;
        ...
        if let Some(successor_id) = result.successor_item_id {
            if let Ok(successor) = workspace.store().get_item(successor_id) {
    ...
    fn begin_done_toggle_or_confirm(
        ...
        self.apply_done_toggle_action(workspace, item_id, was_done, origin, &[])
    }
    ...
    fn batch_toggle_selected_items_done(
        ...
        self.apply_batch_done_action(workspace, &action_item_ids, false, origin)
    }
    ...
    pub(crate) fn apply_batch_done_action(
        ...
        for item_id in item_ids {
            ...
            match workspace.toggle_item_done(*item_id) {
                Ok(_) => {
                    changed += 1;
    ...
    fn cycle_column_summary_fn(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
    ...
    fn cycle_column_numeric_format(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        let slot = match self.current_slot() {
            Some(s) => s,
            None => return Ok(()),
        };
        ...
        workspace.store().update_category(&updated_cat)?;
        self.refresh(workspace.store())?;
        self.status = format!("Column format: {}", describe_numeric_format(&next));
        Ok(())
    }

    fn cycle_current_board_display_mode(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        let Some(mut view) = self.current_view().cloned() else {
            return Ok(());
        };
    ...
    fn toggle_current_view_section_flow(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        let Some(mut view) = self.current_view().cloned() else {
            return Ok(());
        };
    ...
    fn datebook_browse(&mut self, delta: i32, workspace: &Workspace<'_>) -> TuiResult<()> {
        let Some(mut view) = self.current_view().cloned() else {
            return Ok(());
        };
    ...
    where
        F: FnMut(ItemId) -> Result<bool, AgletError>,
    {
    ...
    fn apply_link_wizard(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        ...
        let (status, clear_selection) = match action {
            ...
            LinkWizardAction::ClearDependencies => {
                ...
                for source_item_id in &source_item_ids {
                    let prereqs = workspace.immediate_prereq_ids(*source_item_id)?;
                    let dependents = workspace.immediate_dependent_ids(*source_item_id)?;
                    total_prereqs += prereqs.len();
                    total_dependents += dependents.len();
                    for dependency_id in &prereqs {
                        workspace.unlink_items_depends_on(*source_item_id, *dependency_id)?;
                    }
                    for blocked_id in &dependents {
                        workspace.unlink_items_blocks(*source_item_id, *blocked_id)?;
                    }
                }
    ...
    pub(crate) fn handle_link_wizard_key(
        ...
        match code {
            ...
            KeyCode::Enter => match state.focus {
                ...
                LinkWizardFocus::Confirm => {
                    self.apply_link_wizard(workspace)?;
                }
            },
            _ => {
    ...
    pub(crate) fn handle_normal_key_event(
        ...
        if ctrl_only {
            match key.code {
                ...
                KeyCode::Char('z') => {
                    self.apply_undo(workspace)?;
                    return Ok(false);
                }
                KeyCode::Char('Z') => {
                    self.apply_redo(workspace)?;
                    return Ok(false);
                }
                _ => {}
            }
        }
        self.handle_normal_key(key.code, workspace)
    }
        ...
        if self.board_add_column_create_confirm_open() {
            match inline_create_confirm_key_action(code) {
                InlineCreateConfirmKeyAction::Confirm => {
                    self.confirm_inline_create_board_add_column(workspace)?;
                    return Ok(false);
                }
        ...
        match code {
            ...
            KeyCode::Enter => {
                if let Some(category_id) = self.exact_board_add_column_match_id() {
                    self.insert_board_column_for_category(workspace, category_id)?;
                } else {
                    let matches = self.get_board_add_column_suggest_matches();
    ...
    pub(crate) fn handle_category_direct_edit_key(
        ...
        if self.direct_edit_create_confirm_open() {
            match inline_create_confirm_key_action(code) {
                InlineCreateConfirmKeyAction::Confirm => {
                    self.confirm_inline_create_category_direct_edit(workspace)?;
                    return Ok(false);
                }
                InlineCreateConfirmKeyAction::Cancel => {
                    self.set_direct_edit_create_confirm_name(None);
        ...
        match code {
            ...
            {
                self.apply_category_direct_edit_draft(workspace)?;
                return Ok(false);
            }
            KeyCode::BackTab => {
            ...
            KeyCode::Enter => {
                ...
                if active_text.trim().is_empty() {
                    ...
                    self.assign_selected_suggestion(workspace)?;
                } else {
    ...
    pub(crate) fn open_input_panel_edit_item(&mut self) {
        if let Some(item) = self.selected_item() {
            ...
            let categories: HashSet<aglet_core::model::CategoryId> =
            ...
            for (cat_id, assignment) in &item.assignments {
                ...
                if let Some(cat) = cat {
                    if cat.value_kind == aglet_core::model::CategoryValueKind::Numeric {
    ...
    fn open_item_assign_from_edit_panel(&mut self, workspace: &Workspace<'_>) {
    ...
    pub(crate) fn handle_input_panel_key(
            ...
            })
        {
            ...
            self.open_item_assign_from_edit_panel(workspace);
            return Ok(false);
        }
        ...
        if self.input_panel.as_ref().is_some_and(|p| p.discard_confirm) {
            return self.handle_input_panel_discard_confirm_key(code, workspace);
        }
        ...
        match action {
            InputPanelAction::Save => {
                ...
                match kind {
                    ...
                    input_panel::InputPanelKind::CategoryCreate => {
                        self.save_input_panel_category_create(workspace)?;
                    }
                }
            }
            ...
            InputPanelAction::ToggleCategory => {
                if self.input_panel.as_ref().is_some_and(|panel| {
                    ...
                    let action_index = self
                        ...
                        .unwrap_or(0);
                    match action_index {
                        0 => self.open_item_assign_from_edit_panel(workspace),
                        1 => {
                            if let Some(panel) = &mut self.input_panel {
                                panel.details_popup_open = true;
                                panel.details_scroll = 0;
    ...
    pub(crate) fn save_input_panel_add(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        ...
        let mut process_result = workspace.create_item_cheap(&item, reference_date)?;
            ...
            match Self::parse_when_datetime_input(&when_text) {
                Ok(Some((dt, parsed_rule))) => {
                    ...
                    if let Some(rule) = rule {
                        workspace.set_item_recurrence_rule(item.id, Some(rule))?;
                    }
                }
                Ok(None) => {}
                Err(_) => {
        ...
        if note.is_some() {
            let mut loaded = workspace.store().get_item(item.id)?;
            loaded.note = note;
            loaded.modified_at = Timestamp::now();
            let note_update_result = workspace.update_item_cheap(&loaded, reference_date)?;
            process_result
                .new_assignments
        ...
        for cat_id in &categories_to_assign {
            let _ = workspace.assign_item_manual(
                item.id,
                *cat_id,
                Some("manual:input_panel.add".to_string()),
            );
        ...
        for (cat_id, buf) in &panel.numeric_buffers {
            ...
            if let Ok(val) = trimmed.replace(',', "").parse::<rust_decimal::Decimal>() {
                let _ = workspace.assign_item_numeric_manual(
                    item.id,
                    *cat_id,
                    val,
                    Some("manual:input_panel.add".to_string()),
        ...
        if let Some(view) = self.current_view().cloned() {
            if view.datebook_config.is_none() {
                if let Some(context) = self.current_slot().map(|slot| slot.context.clone()) {
                    self.insert_into_context(workspace, item.id, &view, &context)?;
                }
            }
        }

        let category_names: Vec<String> = workspace
            .store()
            .get_hierarchy()?
            .into_iter()
            .map(|c| c.name)
        ...
        let created = workspace.store().get_item(item.id)?;
        ...
        let _ = self.submit_background_classification(workspace, item.id);
        Ok(())
    }

    /// Save an InputPanel(EditItem) to the store (text, note, when, suggestion decisions).
    pub(crate) fn save_input_panel_edit(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        let Some(panel) = &self.input_panel else {
            self.mode = Mode::Normal;
            return Ok(());
        };
        ...
        let mut item = workspace.store().get_item(item_id)?;
        ...
        let reference_date = jiff::Zoned::now().date();
        let process_result = workspace.update_item_cheap(&item, reference_date)?;
        ...
        if let Some(rule) = recurrence_rule {
            workspace.set_item_recurrence_rule(item_id, Some(rule))?;
        } else if !no_when_change && when_text.is_empty() {
            // Clearing the when field also clears recurrence.
            workspace.set_item_recurrence_rule(item_id, None)?;
        }
        ...
        for (suggestion, decision) in &pending_suggestions {
            match decision {
                SuggestionDecision::Accept => {
                    workspace.accept_classification_suggestion(suggestion.id)?;
                    suggestion_accepted += 1;
                }
                SuggestionDecision::Reject => {
                    workspace.reject_classification_suggestion(suggestion.id)?;
        ...
        self.status = status;
        // Submit background classification (Ollama) if enabled.
        let _ = self.submit_background_classification(workspace, item_id);
        Ok(())
    }
    ...
    fn parse_when_datetime_input_with_reference_date(
        ...
        use aglet_core::dates::DateParseResult;
    ...
    fn save_input_panel_name(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        let input_text = self
            .input_panel
            .as_ref()
        ...
        match self.name_input_context {
            Some(NameInputContext::ViewClone) => {
                ...
                match workspace.store().clone_view(source_id, name.clone()) {
                    Ok(_cloned) => {
                        self.refresh(workspace.store())?;
                        self.set_view_selection_by_name(&name);
                        self.input_panel = None;
            ...
            Some(NameInputContext::ViewRename) => {
                ...
                if aglet_core::store::is_system_view_name(&view.name) {
                    self.input_panel = None;
                    self.name_input_context = None;
                    self.view_pending_edit_name = None;
                    self.mode = Mode::ViewPicker;
                ...
                match workspace.store().update_view(&view) {
                    Ok(()) => {
                        self.refresh(workspace.store())?;
                        self.set_view_selection_by_name(&name);
                        self.input_panel = None;
            ...
            Some(NameInputContext::NumericValueEdit) => {
                ...
                match normalized.parse::<rust_decimal::Decimal>() {
                    Ok(decimal_value) => {
                        match workspace.assign_item_numeric_manual(
                            target.item_id,
                            target.category_id,
                            decimal_value,
            ...
            Some(NameInputContext::WhenDateEdit) => {
                ...
                match workspace.set_item_when_date(
                    target.item_id,
                    parsed_when,
                    ...
                    Ok(_result) => {
                        ...
                        if inline_rule.is_some() || parsed_when.is_none() {
                            let _ = workspace.set_item_recurrence_rule(target.item_id, inline_rule);
                        }
                        self.refresh(workspace.store())?;
            ...
            Some(NameInputContext::OllamaBaseUrl) => {
                ...
                workspace.store().set_classification_config(&config)?;
                self.refresh(workspace.store())?;
                self.input_panel = None;
                self.name_input_context = None;
    ...
    fn save_input_panel_category_create(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        let Some(panel) = &self.input_panel else {
            self.mode = self.name_input_return_mode();
            self.name_input_context = None;
            return Ok(());
        ...
        match workspace.create_category(&category) {
            Ok(result) => {
                self.refresh(workspace.store())?;
    ...
    fn item_assign_reapply_status(
        ...
        let hypothetical = workspace.preview_manual_category_toggle(item_id, category_id)?;
    ...
    pub(crate) fn handle_item_assign_category_key(
        ...
        if self.item_assign_pane == ItemAssignPane::ViewSection {
            return self.handle_item_assign_view_key(code, workspace);
        }
        ...
        match code {
            ...
            KeyCode::Char(' ') => {
                ...
                if batch_mode && action_item_ids.len() > 1 {
                    ...
                    if let Some(reason) =
                        self.item_assign_reapply_status(workspace, item_id, row.id, &row.name)?
                    {
                        self.status = reason;
                        return Ok(false);
                    }
                    ...
                    let old_assignment = self
                        ...
                        .cloned();
                    match workspace.unassign_item_manual(item_id, row.id) {
                        Ok(result) => {
                    ...
                    let result = workspace.assign_item_manual(
                        item_id,
                        row.id,
                        Some("manual:tui.assign".to_string()),
                    )?;
            ...
            KeyCode::Enter => {
                ...
                if self.item_assign_dirty {
                    ...
                    self.handle_item_assign_category_key(KeyCode::Char(' '), workspace)?;
                    if self.mode == Mode::ItemAssignPicker {
                        self.close_item_assign_session(self.status.clone());
                    }
                }
        ...
        match code {
            ...
            KeyCode::Char('r') => {
                ...
                for item_id in &action_ids {
                    ...
                    match workspace.remove_item_from_view(*item_id, &view) {
                        Ok(_) => changed += 1,
                        Err(err) => {
                            if first_error.is_none() {
                                first_error = Some(err.to_string());
            ...
            KeyCode::Enter => {
                if self.item_assign_dirty {
                    ...
                    self.handle_item_assign_view_key(KeyCode::Char(' '), workspace)?;
                    if self.mode == Mode::ItemAssignPicker {
                        self.close_item_assign_session(self.status.clone());
                    }
                }
    ...
    pub(crate) fn handle_inspect_unassign_key(
        ...
        match code {
            ...
            KeyCode::Enter => {
                ...
                workspace.unassign_item_manual(item_id, row.category_id)?;
                self.refresh(workspace.store())?;
                self.set_item_selection_by_id(item_id);
                self.mode = Mode::Normal;
...
mod tests {
    ...
    use aglet_core::model::CategoryId;
    use jiff::civil::Date;
    ...
    fn input_panel_category_filter_narrows_rows_and_toggles_match() {
        ...
        app.handle_input_panel_key(KeyCode::Char('/'), &workspace)
            .expect("open filter");
        app.handle_input_panel_key(KeyCode::Char('b'), &workspace)
            .expect("type filter");
        assert_eq!(
    ...
    fn ctrl_g_sets_pending_external_edit_for_text() {
        ...
        let classifier = SubstringClassifier;
        let workspace = Workspace::new(&store, &classifier);
        app.handle_input_panel_key(KeyCode::Char('g'), &workspace)
            .expect("ctrl+g");
        assert_eq!(
