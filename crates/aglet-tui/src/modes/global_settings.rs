...
impl App {
    ...
    fn cycle_global_settings_auto_refresh(
        ...
        self.persist_auto_refresh_interval(workspace.store())?;
    ...
    fn cycle_global_settings_section_borders(
        ...
        self.persist_section_border_mode(workspace.store())?;
        self.status = format!("Section borders: {}", self.section_border_mode_label());
        Ok(())
    }

    fn toggle_global_settings_note_glyphs(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        self.show_note_glyphs = !self.show_note_glyphs;
        self.persist_show_note_glyphs(workspace.store())?;
    ...
    pub(crate) fn handle_global_settings_key(
        ...
        if self.settings.ollama_model_picker.is_some() {
            self.handle_ollama_model_picker_key(code, workspace)?;
            return Ok(false);
        }
        if self
            .settings
        ...
        match code {
            ...
            KeyCode::Right | KeyCode::Char('l') | KeyCode::Char(' ') => {
                match self.global_settings_selected_kind() {
                    GlobalSettingsRow::AutoRefresh => {
                        self.cycle_global_settings_auto_refresh(workspace, true)?;
                    }
                    GlobalSettingsRow::SectionBorders => {
                        self.cycle_global_settings_section_borders(workspace, true)?;
                    }
                    GlobalSettingsRow::NoteGlyphs => {
                        self.toggle_global_settings_note_glyphs(workspace)?;
                    }
                    GlobalSettingsRow::LiteralClassificationMode => {
                        self.cycle_global_settings_literal_mode(workspace, true)?;
                    }
                    GlobalSettingsRow::SemanticClassificationMode => {
                        self.cycle_global_settings_semantic_mode(workspace, true)?;
                    }
                    GlobalSettingsRow::SemanticProvider => {
                        self.cycle_global_settings_semantic_provider(workspace, true)?;
                    }
            ...
            KeyCode::Left | KeyCode::Char('h') => match self.global_settings_selected_kind() {
                GlobalSettingsRow::AutoRefresh => {
                    self.cycle_global_settings_auto_refresh(workspace, false)?;
                }
                GlobalSettingsRow::SectionBorders => {
                    self.cycle_global_settings_section_borders(workspace, false)?;
                }
                GlobalSettingsRow::NoteGlyphs => {
                    self.toggle_global_settings_note_glyphs(workspace)?;
                }
                GlobalSettingsRow::LiteralClassificationMode => {
                    self.cycle_global_settings_literal_mode(workspace, false)?;
                }
                GlobalSettingsRow::SemanticClassificationMode => {
                    self.cycle_global_settings_semantic_mode(workspace, false)?;
                }
                GlobalSettingsRow::SemanticProvider => {
                    self.cycle_global_settings_semantic_provider(workspace, false)?;
                }
                GlobalSettingsRow::OllamaBaseUrl
                | GlobalSettingsRow::OllamaModel
                | GlobalSettingsRow::OllamaTimeout
            ...
            KeyCode::Enter => match self.global_settings_selected_kind() {
                ...
                GlobalSettingsRow::OllamaModel => {
                    self.open_ollama_model_picker(workspace);
                }
                GlobalSettingsRow::OllamaTimeout => {
    ...
    fn open_ollama_model_picker(&mut self, _agenda: &Workspace<'_>) {
