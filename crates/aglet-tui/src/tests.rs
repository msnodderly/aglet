...
fn category_direct_edit_enter_prefers_exact_match_over_highlighted_suggestion() {
    ...
    app.handle_category_direct_edit_key(KeyCode::Enter, &aglet)
    ...
    app.handle_category_direct_edit_key(KeyCode::Tab, &aglet)
        .expect("tab away from Input");
    app.handle_category_direct_edit_key(KeyCode::Char('S'), &aglet)
        .expect("save draft");
...
fn category_direct_edit_input_focus_allows_typing_n_a_x() {
    ...
    app.handle_category_direct_edit_key(KeyCode::Char('n'), &aglet)
        .expect("type n");
    app.handle_category_direct_edit_key(KeyCode::Char('a'), &aglet)
        .expect("type a");
    app.handle_category_direct_edit_key(KeyCode::Char('x'), &aglet)
        .expect("type x");
...
fn category_direct_edit_input_focus_plus_adds_row_without_switching_focus() {
    ...
    app.handle_category_direct_edit_key(KeyCode::Tab, &aglet)
        .expect("tab away from Input");
    app.handle_category_direct_edit_key(KeyCode::Char('+'), &aglet)
...
fn category_direct_edit_entries_focus_n_and_x_still_add_and_remove_rows() {
    ...
    app.handle_category_direct_edit_key(KeyCode::BackTab, &aglet)
...
fn category_direct_edit_tab_cycles_focus_instead_of_autocomplete_from_suggestions() {
    ...
    app.handle_category_direct_edit_key(KeyCode::Char('A'), &aglet)
        .expect("type A");
    app.handle_category_direct_edit_key(KeyCode::Char('l'), &aglet)
        .expect("type l");
...
fn category_direct_edit_right_autocompletes_from_suggestions_focus() {
    ...
    app.handle_category_direct_edit_key(KeyCode::Right, &aglet)
        .expect("autocomplete");
    let state = app.category_direct_edit_state().expect("state");
    assert_eq!(state.focus, CategoryDirectEditFocus::Suggestions);
...
fn category_direct_edit_inline_create_confirm_resolves_row_and_stays_open() {
    ...
    app.handle_category_direct_edit_key(KeyCode::Char('y'), &aglet)
...
fn board_cell_enter_opens_category_column_picker_for_non_exclusive_parent() {
    ...
    app.handle_key(KeyCode::Enter, &aglet).expect("enter");
...
fn category_column_picker_multi_toggle_save_applies_diff_and_preserves_other_assignments() {
    ...
    for ch in "CLI".chars() {
        app.handle_key(KeyCode::Char(ch), &aglet).expect("type cli");
    }
    app.handle_key(KeyCode::Tab, &aglet).expect("focus list");
    app.handle_key(KeyCode::Char(' '), &aglet)
        .expect("toggle cli on");

    app.handle_key(KeyCode::Tab, &aglet).expect("focus filter");
    for _ in 0..3 {
        app.handle_key(KeyCode::Backspace, &aglet)
            .expect("clear filter");
    }
    for ch in "UX".chars() {
        app.handle_key(KeyCode::Char(ch), &aglet).expect("type ux");
    }
    app.handle_key(KeyCode::Tab, &aglet).expect("focus list");
    app.handle_key(KeyCode::Char(' '), &aglet)
        .expect("toggle ux off");

    app.handle_key(KeyCode::Enter, &aglet).expect("save picker");
    assert_eq!(app.mode, Mode::Normal);
...
fn category_column_picker_cancel_discards_staged_changes() {
    ...
    app.handle_key(KeyCode::Enter, &aglet).expect("open picker");
    for ch in "CLI".chars() {
        app.handle_key(KeyCode::Char(ch), &aglet).expect("type");
    }
    app.handle_key(KeyCode::Char(' '), &aglet)
        .expect("toggle on");
    app.handle_key(KeyCode::Esc, &aglet).expect("cancel");
...
fn board_add_column_picker_render_survives_empty_matches_after_typing() {
    ...
    app.handle_key(KeyCode::Char('+'), &aglet)
        .expect("open picker");
    app.handle_key(KeyCode::Char('z'), &aglet)
...
fn link_wizard_excludes_closed_targets_and_restores_after_reopen() {
    ...
    aglet.mark_item_done(done_target.id).expect("mark done");
    aglet
        .assign_item_manual(
            complete_target.id,
            complete.id,
    ...
    app.handle_key(KeyCode::Esc, &aglet)
...
fn link_wizard_target_navigation_does_not_wrap() {
    ...
    for _ in 0..8 {
        app.handle_key(KeyCode::Up, &aglet)
...
fn board_column_reorder_shift_keys_move_item_column() {
    ...
    app.handle_key(KeyCode::Char('H'), &aglet)
    ...
    app.handle_key(KeyCode::Char('L'), &aglet)
...
fn board_column_reorder_and_remove_visidata_keys_update_columns() {
    ...
    app.handle_key(KeyCode::Char('y'), &aglet)
...
fn board_g_prefix_rejects_removed_gh_gl_bindings() {
    ...
    app.handle_key(KeyCode::Char('g'), &aglet)
        .expect("g prefix");
    app.handle_key(KeyCode::Char('H'), &aglet)
...
fn category_manager_details_cursor_position_tracks_numeric_inline_input() {
    ...
    app.handle_category_manager_key(KeyCode::Enter, &aglet)
...
fn category_manager_condition_cursor_position_tracks_date_value_input() {
    ...
    app.handle_category_manager_key(KeyCode::Char('d'), &aglet)
...
fn input_panel_edit_enter_in_note_inserts_newline() {
    ...
    app.handle_normal_key(KeyCode::Enter, &aglet)
    ...
    app.handle_input_panel_key(KeyCode::Enter, &aglet)
...
fn view_picker_delete_uses_x_and_removes_selected_view() {
    ...
    app.handle_view_picker_key(KeyCode::Char('x'), &aglet)
        .expect("open delete confirm");
    assert_eq!(app.mode, Mode::ViewDeleteConfirm);

    app.handle_view_delete_key(KeyCode::Char('y'), &aglet)
    ...
    app.handle_view_picker_key(KeyCode::Char('d'), &aglet)
...
fn view_picker_v_opens_view_edit() {
    ...
    app.handle_view_picker_key(KeyCode::Char('V'), &aglet)
...
fn view_picker_enter_switches_to_ready_queue_and_filters_blocked_items() {
    ...
    app.handle_view_picker_key(KeyCode::Enter, &aglet)
...
fn view_picker_blocks_edit_delete_and_rename_for_all_items() {
    ...
    app.handle_view_picker_key(KeyCode::Char('e'), &aglet)
        .expect("edit key");
    assert_eq!(app.mode, Mode::ViewPicker);
    ...
    app.handle_view_picker_key(KeyCode::Char('r'), &aglet)
        .expect("rename key");
    assert_eq!(app.mode, Mode::ViewPicker);
    assert!(app.status.contains("immutable"));
...
fn view_picker_lowercase_n_opens_view_create() {
    ...
    app.handle_view_picker_key(KeyCode::Char('n'), &aglet)
...
fn view_picker_c_opens_clone_name_input() {
    ...
    app.handle_view_picker_key(KeyCode::Char('c'), &aglet)
...
fn view_picker_clone_creates_new_view() {
    ...
    for ch in "Cloned View".chars() {
        app.handle_input_panel_key(KeyCode::Char(ch), &aglet)
...
fn view_create_name_save_opens_unsaved_view_wizard_with_first_section_editing() {
    ...
    for ch in "Mixed".chars() {
        app.handle_view_edit_key(KeyCode::Char(ch), &aglet)
            .expect("type view name");
    }
    // Enter confirms the name
    app.handle_view_edit_key(KeyCode::Enter, &aglet)
        .expect("confirm name");
...
fn view_create_wizard_esc_discards_unsaved_view() {
    ...
    app.handle_view_edit_key(KeyCode::Esc, &aglet)
    ...
    app.handle_view_edit_key(KeyCode::Char('n'), &aglet)
...
fn view_create_wizard_s_save_persists_view() {
    ...
    // Save the new view
    app.handle_view_edit_key(KeyCode::Char('S'), &aglet)
...
fn normal_mode_u_toggles_hide_dependent_items_session_only() {
    ...
    app.handle_normal_key(KeyCode::Char('u'), &aglet)
...
fn inspect_unassign_picker_jk_tracks_assignment_rows() {
    ...
    app.handle_inspect_unassign_key(KeyCode::Char('j'), &aglet)
    ...
    app.handle_inspect_unassign_key(KeyCode::Char('k'), &aglet)
...
fn normal_mode_f_cycles_numeric_column_format() {
    ...
    app.handle_normal_key(KeyCode::Char('f'), &aglet)
        .expect("press f");
...
fn normal_mode_tab_and_backtab_switch_sections_without_wrapping() {
    ...
    app.handle_normal_key(KeyCode::Tab, &aglet)
    ...
    app.handle_normal_key(KeyCode::BackTab, &aglet)
    ...
    app.handle_normal_key(KeyCode::Char('J'), &aglet)
    ...
    app.handle_normal_key(KeyCode::Char('K'), &aglet)
...
fn normal_mode_p_and_i_manage_preview_modes() {
    ...
    app.handle_normal_key(KeyCode::Char('p'), &aglet)
        .expect("open preview");
    assert!(app.show_preview);
    assert_eq!(app.preview_mode, super::PreviewMode::Summary);

    app.handle_normal_key(KeyCode::Char('i'), &aglet)
...
fn normal_mode_z_toggles_view_card_display_mode() {
    ...
    app.handle_normal_key(KeyCode::Char('z'), &aglet)
...
fn normal_mode_m_toggles_view_section_flow() {
    ...
    app.handle_normal_key(KeyCode::Char('m'), &aglet)
...
fn global_settings_auto_refresh_row_cycles_interval_with_keys() {
    ...
    app.handle_normal_key(KeyCode::Char('g'), &aglet)
...
fn auto_refresh_timer_reloads_in_normal_mode_when_due() {
    ...
    app.maybe_run_auto_refresh(&aglet)
...
fn normal_mode_footer_hints_show_numeric_shortcuts_only_for_numeric_column_focus() {
    ...
    let _aglet = Aglet::new(&store, &classifier);
...
fn global_settings_ollama_text_rows_persist_updates() {
    ...
    select_global_settings_row_for_test(&mut app, &aglet, GlobalSettingsRow::OllamaBaseUrl);
    app.handle_key(KeyCode::Enter, &aglet)
    ...
    select_global_settings_row_for_test(&mut app, &aglet, GlobalSettingsRow::OllamaModel);
    app.handle_key(KeyCode::Enter, &aglet)
...
fn ollama_model_picker_esc_cancels() {
    ...
    app.handle_key(KeyCode::Esc, &aglet).expect("picker esc");
    assert!(app.settings.ollama_model_picker.is_none());
...
fn category_manager_tab_cycles_details_sections_before_returning_to_filter() {
    ...
    app.handle_category_manager_key(KeyCode::Tab, &aglet)
    ...
    app.handle_category_manager_key(KeyCode::BackTab, &aglet)
...
fn category_manager_conditions_focus_shows_rule_count() {
    ...
    loop {
        ...
        app.handle_category_manager_key(KeyCode::Down, &aglet)
...
fn category_manager_condition_edit_add_and_save() {
    ...
    app.handle_category_manager_key(KeyCode::Char('a'), &aglet)
    ...
    loop {
        ...
        app.handle_category_manager_key(KeyCode::Char('j'), &aglet)
    ...
    app.handle_category_manager_key(KeyCode::Esc, &aglet)
...
fn category_manager_condition_edit_delete_condition() {
    ...
    app.handle_category_manager_key(KeyCode::Char('x'), &aglet)
...
fn category_manager_date_condition_match_cycle_includes_range() {
    ...
    for _ in 0..4 {
        app.handle_category_manager_key(KeyCode::Char('l'), &aglet)
            .expect("cycle match");
    }
...
fn category_manager_date_condition_range_fields_accept_direct_typing() {
    ...
    for key in [
        ...
        app.handle_category_manager_key(key, &aglet)
...
fn category_manager_condition_list_can_toggle_any_all_mode() {
    ...
    app.handle_category_manager_key(KeyCode::Char('m'), &aglet)
...
fn category_manager_action_edit_can_switch_existing_action_kind() {
    ...
    assert!(app.category_manager_action_edit().unwrap().picker_open);
    app.handle_category_manager_key(KeyCode::Char('2'), &aglet)
...
fn normal_mode_space_toggles_selection_and_esc_clears_before_filter() {
    ...
    app.handle_normal_key(KeyCode::Esc, &aglet)
...
fn refresh_prunes_selected_items_that_are_no_longer_visible() {
    ...
    aglet.delete_item(item.id, "test").expect("delete item");
...
fn cycle_view_clears_transient_selection() {
    ...
    app.cycle_view(1, &aglet).expect("cycle to next view");
    assert_eq!(app.selected_count(), 0);
}
...
fn item_assign_picker_backtab_cycles_between_panes() {
    ...
    app.handle_item_assign_category_key(KeyCode::BackTab, &aglet)
...
fn batch_assign_picker_esc_preserves_selection_without_changes() {
    ...
    app.handle_normal_key(KeyCode::Char('a'), &aglet)
        .expect("open batch assign picker");
    app.handle_item_assign_category_key(KeyCode::Esc, &aglet)
...
fn assign_picker_view_unmatched_row_inserts_item_into_view() {
    ...
    app.handle_normal_key(KeyCode::Char('a'), &aglet)
        .expect("open assign picker");
    app.handle_item_assign_category_key(KeyCode::Tab, &aglet)
...
fn assign_picker_view_repeated_space_keeps_operating_on_original_item() {
    ...
    current_view
        .criteria
        .set_criterion(CriterionMode::And, aglet_cat.id);
    store
    ...
    store.create_item(&second).expect("create second");
    aglet
        .assign_item_manual(first.id, aglet_cat.id, Some("manual:test".to_string()))
        .expect("assign first to current view");
    aglet
        .assign_item_manual(second.id, aglet_cat.id, Some("manual:test".to_string()))
...
fn assign_picker_view_remove_skips_items_absent_from_target_view() {
    ...
    app.handle_item_assign_category_key(KeyCode::Char('r'), &aglet)
...
fn normal_mode_x_with_selection_opens_batch_delete_confirm() {
    ...
    app.handle_normal_key(KeyCode::Char(' '), &aglet)
        .expect("select second");
    app.handle_normal_key(KeyCode::Char('x'), &aglet)
...
fn batch_delete_confirm_y_deletes_selected_items_and_clears_selection() {
    ...
    app.handle_normal_key(KeyCode::Char('x'), &aglet)
        .expect("open batch delete confirm");
    app.handle_confirm_delete_key(KeyCode::Char('y'), &aglet)
...
fn batch_delete_confirm_esc_preserves_selected_items() {
    ...
    app.handle_normal_key(KeyCode::Char('x'), &aglet)
        .expect("open batch delete confirm");
    app.handle_confirm_delete_key(KeyCode::Esc, &aglet)
...
fn item_info_contains_link_sections_while_summary_stays_primary() {
    ...
    aglet.link_items_blocks(c.id, a.id).expect("C blocks A");
    aglet.link_items_related(a.id, d.id).expect("A related D");
...
fn category_create_panel_opens_and_creates_root_category() {
    ...
    app.handle_normal_key(KeyCode::Char('c'), &aglet)
        .expect("open category manager");
    app.handle_category_manager_key(KeyCode::Char('n'), &aglet)
...
fn category_create_panel_numeric_via_type_toggle() {
    ...
    app.handle_input_panel_key(KeyCode::Char(' '), &aglet)
...
fn category_manager_inline_rename_avoids_input_panel_and_updates_name() {
    ...
    app.handle_normal_key(KeyCode::Char('c'), &aglet)
        .expect("open category manager");
    app.set_category_selection_by_id(category.id);
    app.handle_category_manager_key(KeyCode::Char('r'), &aglet)
        .expect("start rename");
    assert!(
    ...
    for _ in 0.."Work".len() {
        app.handle_category_manager_key(KeyCode::Backspace, &aglet)
            .expect("clear rename buffer");
    }
    for c in "Office".chars() {
        app.handle_category_manager_key(KeyCode::Char(c), &aglet)
            .expect("type rename");
    }
    app.handle_category_manager_key(KeyCode::Enter, &aglet)
        .expect("apply rename");
...
fn category_manager_inline_delete_confirm_stays_in_manager_mode() {
    ...
    app.handle_category_manager_key(KeyCode::Char('y'), &aglet)
        .expect("confirm delete");
    assert_eq!(app.mode, Mode::CategoryManager);
    assert!(matches!(
...
fn category_manager_filter_focus_types_text_instead_of_triggering_commands() {
    ...
    app.handle_category_manager_key(KeyCode::Char('o'), &aglet)
...
fn category_manager_tab_focuses_details_and_p_no_longer_opens_reparent_ui() {
    ...
    app.handle_category_manager_key(KeyCode::Char('p'), &aglet)
...
fn category_manager_upper_k_reorders_selected_category_up_among_siblings() {
    ...
    app.handle_category_manager_key(KeyCode::Char('K'), &aglet)
        .expect("reorder up");
...
fn category_manager_upper_h_outdents_selected_category_after_parent() {
    ...
    app.handle_category_manager_key(KeyCode::Char('H'), &aglet)
        .expect("outdent");
...
fn category_manager_details_also_match_save_persists_one_entry_per_line() {
    ...
    for code in [
        ...
        app.handle_category_manager_key(code, &aglet)
    ...
    app.handle_category_manager_key(KeyCode::Char('S'), &aglet)
...
fn category_manager_details_and_quick_flag_toggles_work_inline() {
    ...
    app.handle_category_manager_key(KeyCode::Char('i'), &aglet)
        .expect("quick toggle auto-match");
    app.handle_category_manager_key(KeyCode::Char('g'), &aglet)
...
fn open_global_settings_for_test(app: &mut App, aglet: &Aglet<'_>) {
    app.mode = Mode::Normal;
    app.handle_normal_key(KeyCode::Char('g'), aglet)
        .expect("g prefix should start");
    app.handle_normal_key(KeyCode::Char('s'), aglet)
...
fn select_workflow_picker_category_for_test(app: &mut App, aglet: &Aglet<'_>, target_name: &str) {
...
fn workflow_ready_role_assignment_disables_implicit_match() {
    ...
    app.refresh(&store).expect("refresh app");
    open_global_settings_for_test(&mut app, &aglet);
    select_global_settings_row_for_test(&mut app, &aglet, GlobalSettingsRow::WorkflowReady);

    app.handle_key(KeyCode::Enter, &aglet)
        .expect("open ready queue picker");
    select_workflow_picker_category_for_test(&mut app, &aglet, "Ready");
    app.handle_key(KeyCode::Enter, &aglet)
...
fn workflow_claim_role_assignment_disables_implicit_match() {
    ...
    app.refresh(&store).expect("refresh app");
    open_global_settings_for_test(&mut app, &aglet);
    select_global_settings_row_for_test(&mut app, &aglet, GlobalSettingsRow::WorkflowClaim);
...
fn workflow_role_disable_does_not_reenable_implicit_match() {
    ...
    app.handle_key(KeyCode::Char('x'), &aglet)
...
fn global_settings_workflow_picker_scroll_offset_stays_stable_while_selection_is_visible() {
    ...
    app.handle_key(KeyCode::Char('k'), &aglet)
...
fn normal_mode_f10_opens_global_settings() {
    ...
    app.handle_normal_key(KeyCode::F(10), &aglet)
...
fn global_search_enter_opens_top_visible_item_and_esc_restores_previous_view() {
    ...
    for ch in "buy".chars() {
        app.handle_search_bar_key(KeyCode::Char(ch), &aglet)
...
fn normal_mode_d_toggles_done_state() {
    ...
    app.handle_normal_key(KeyCode::Char('d'), &aglet)
...
fn batch_done_all_done_marks_selected_items_not_done() {
    ...
    aglet
        .assign_item_manual(second.id, work.id, Some("manual:test".to_string()))
        .expect("assign second");
    aglet.mark_item_done(first.id).expect("mark first done");
    aglet.mark_item_done(second.id).expect("mark second done");
...
fn batch_done_confirm_n_marks_done_and_keeps_links() {
    ...
    app.handle_normal_key(KeyCode::Char('d'), &aglet)
        .expect("batch done");
    app.handle_confirm_delete_key(KeyCode::Char('n'), &aglet)
...
fn normal_v_then_e_opens_view_edit_for_active_view() {
    ...
    app.handle_key(KeyCode::Char('v'), &aglet)
...
fn view_edit_criteria_rows_render_in_draft_order_and_space_toggles_selected_row() {
    ...
    app.handle_view_edit_key(KeyCode::Char(' '), &aglet)
...
fn view_edit_picker_space_cycles_criterion_mode() {
    ...
    // '3' key: sets Or
    app.handle_view_edit_key(KeyCode::Char('3'), &aglet)
    ...
    // '0' key: clears
    app.handle_view_edit_key(KeyCode::Char('0'), &aglet)
...
fn view_edit_tab_cycles_panes() {
    ...
    app.handle_view_edit_key(KeyCode::Tab, &aglet).expect("tab");
    assert_eq!(
        app.view_edit_state.as_ref().unwrap().pane_focus,
    ...
    app.handle_view_edit_key(KeyCode::Tab, &aglet)
...
fn view_edit_shift_tab_cycles_panes_backwards() {
    ...
    app.handle_view_edit_key(KeyCode::BackTab, &aglet)
        .expect("shift-tab");
    assert_eq!(
        app.view_edit_state.as_ref().unwrap().pane_focus,
        ViewEditPaneFocus::Details
...
fn view_edit_esc_returns_to_view_picker() {
    ...
    app.handle_view_edit_key(KeyCode::Esc, &aglet).expect("esc");
...
fn view_edit_esc_on_dirty_prompts_save_confirm() {
    ...
    app.handle_view_edit_key(KeyCode::Char('m'), &aglet)
...
fn view_edit_inline_input_intercepts_keys_before_region() {
    ...
    app.handle_view_edit_key(KeyCode::Char('j'), &aglet)
...
fn view_edit_details_jk_moves_between_criteria_and_view_aux_rows() {
    ...
    app.handle_view_edit_key(KeyCode::Char('l'), &aglet)
    ...
    app.handle_view_edit_key(KeyCode::Char('k'), &aglet)
    ...
    app.handle_view_edit_key(KeyCode::Char('h'), &aglet)
...
fn view_edit_alias_row_enter_opens_alias_picker_and_saves_value() {
    ...
    app.handle_view_edit_key(KeyCode::Char('A'), &aglet)
...
fn view_edit_section_x_prompts_before_delete_and_y_confirms() {
    ...
    app.handle_view_edit_key(KeyCode::Char('y'), &aglet)
        .expect("confirm delete");
    assert_eq!(
        app.view_edit_state.as_ref().unwrap().draft.sections.len(),
        0
...
fn view_edit_section_plus_opens_criteria_picker_without_pre_expand() {
    ...
    app.handle_view_edit_key(KeyCode::Char('f'), &aglet)
        .expect("open section criteria picker");
    assert!(matches!(
        app.view_edit_state.as_ref().unwrap().overlay,
        Some(super::ViewEditOverlay::CategoryPicker {
...
fn view_edit_section_e_starts_section_title_edit() {
    ...
    app.handle_view_edit_key(KeyCode::Enter, &aglet)
        .expect("confirm default section title");
    app.handle_view_edit_key(KeyCode::Char('e'), &aglet)
...
fn view_edit_view_row_r_starts_name_edit_and_enter_saves_draft_name() {
    ...
    app.handle_view_edit_key(KeyCode::Char('r'), &aglet)
...
fn view_edit_slash_opens_section_filter_and_esc_clears_filter_before_close() {
    ...
    app.handle_view_edit_key(KeyCode::Char('b'), &aglet)
        .expect("type filter");
    assert_eq!(
        app.view_edit_state
            .as_ref()
...
fn view_edit_p_toggles_preview_and_tab_cycles_preview_pane() {
    ...
    app.handle_view_edit_key(KeyCode::Char('p'), &aglet)
        .expect("hide preview");
    assert!(!app.view_edit_state.as_ref().unwrap().preview_visible);
...
fn view_edit_section_c_opens_columns_picker_and_toggles_column() {
    ...
    app.handle_view_edit_key(KeyCode::Char('c'), &aglet)
        .expect("open section columns picker");
    assert!(matches!(
        app.view_edit_state.as_ref().unwrap().overlay,
        Some(super::ViewEditOverlay::CategoryPicker {
...
fn section_column_picker_shows_but_blocks_leaf_tag_headings() {
    ...
    app.handle_view_edit_key(KeyCode::Tab, &aglet).unwrap();
    app.handle_view_edit_key(KeyCode::Char('n'), &aglet)
        .unwrap();
    app.handle_view_edit_key(KeyCode::Enter, &aglet).unwrap();
    app.handle_view_edit_key(KeyCode::Char('c'), &aglet)
        .unwrap();
...
fn normal_mode_shift_down_moves_item_to_next_section_like_right_bracket() {
    ...
    app.handle_key_event(KeyEvent::new(KeyCode::Down, KeyModifiers::SHIFT), &aglet)
...
fn normal_mode_shift_up_moves_item_to_previous_section_like_left_bracket() {
    ...
    app.handle_key_event(KeyEvent::new(KeyCode::Up, KeyModifiers::SHIFT), &aglet)
...
fn horizontal_section_flow_hl_moves_between_sections_and_restores_lane_item_index() {
    ...
    app.handle_normal_key(KeyCode::Char('l'), &aglet)
    ...
    app.handle_normal_key(KeyCode::Char('h'), &aglet)
...
fn section_filters_are_independent() {
    ...
    app.handle_search_bar_key(KeyCode::Down, &aglet)
...
fn search_bar_esc_clears_filter_and_buffer() {
    ...
    app.handle_search_bar_key(KeyCode::Esc, &aglet)
        .expect("esc clears");
    assert_eq!(app.mode, Mode::Normal);
    assert!(
...
fn section_filters_reset_on_view_switch() {
    ...
    app.cycle_view(1, &aglet).expect("cycle view");
...
fn search_bar_live_filtering() {
    ...
    app.handle_search_bar_key(KeyCode::Char('f'), &aglet)
        .expect("type f");
    assert_eq!(
...
fn edit_item_details_popup_opens_and_scrolls() {
    ...
    app.handle_key(KeyCode::PageDown, &aglet)
...
fn moving_from_unmatched_to_section_preserves_view_remove_side_effect_categories() {
    ...
    app.slot_index = 1;
    app.item_index = 0;
    app.move_selected_item_between_slots(-1, &aglet)
