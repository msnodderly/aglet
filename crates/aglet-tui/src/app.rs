...
use aglet_core::classification::PROVIDER_ID_IMPLICIT_STRING;
use aglet_core::workflow::{blocked_item_ids, READY_QUEUE_VIEW_NAME};
...
impl App {
    ...
    fn maybe_run_temporal_reevaluation(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        if !workspace.has_date_conditions()? {
            return Ok(());
        }
        ...
        let _ = workspace.reevaluate_temporal_conditions()?;
    ...
    pub(crate) fn maybe_run_auto_refresh(&mut self, workspace: &Workspace<'_>) -> TuiResult<bool> {
        if self.should_auto_refresh_now() {
            self.maybe_run_temporal_reevaluation(workspace)?;
    ...
    pub(crate) fn run(
        ...
        self.maybe_run_temporal_reevaluation(workspace)?;
        self.refresh(workspace.store())?;
        self.load_last_view_name(workspace.store())?;
        self.refresh(workspace.store())?; // re-resolve slots for the restored view
        self.load_auto_refresh_interval(workspace.store())?;
        self.load_section_border_mode(workspace.store())?;
        self.load_show_note_glyphs(workspace.store())?;
        self.auto_refresh_last_tick = Instant::now();
        ...
        loop {
            ...
            needs_redraw |= self.process_classification_results(workspace)?;
            if needs_redraw {
            ...
            if !event::poll(std::time::Duration::from_millis(200))? {
                needs_redraw |= self.maybe_run_auto_refresh(workspace)?;
                continue;
            }
            ...
            match event::read()? {
                ...
                Event::Key(key) => {
                    ...
                    let should_quit = match self.handle_key_event(key, workspace) {
                        Ok(value) => value,
                        Err(err) => {
                    ...
                    needs_redraw = true;
                    if should_quit {
                        let _ = self.persist_last_view_name(workspace.store());
                        break;
                    }
    ...
    pub(crate) fn refresh(&mut self, store: &Store) -> TuiResult<()> {
        ...
        for item in &items {
            let links = aglet_core::model::ItemLinksForItem {
    ...
    fn rebuild_classification_ui(&mut self, store: &Store) -> TuiResult<()> {
        ...
        let mut review_items: Vec<ClassificationReviewItem> = grouped
            ...
            .map(|(item_id, mut suggestions)| {
                suggestions.sort_by_key(|left| left.created_at);
    ...
    pub(crate) fn process_classification_results(
        ...
        while let Some(result) = self.classification.worker.try_recv() {
            ...
            let current_item = match workspace.store().get_item(result.item_id) {
                Ok(item) => item,
                Err(_) => continue, // item was deleted
            };
            let current_hash = aglet_core::classification::item_revision_hash(&current_item);
            ...
            let queued = workspace.apply_classification_results(
                result.item_id,
    ...
    pub(crate) fn submit_background_classification(
        ...
        if let Some(job) = workspace.prepare_background_classification(item_id, reference_date)? {
            self.classification
                .in_flight_classifications
                .insert(item_id);
            self.debug_log_classification(
    ...
    fn debug_log_classification(&self, workspace: &Workspace<'_>, message: &str) {
        if !workspace.debug_enabled() {
            return;
        }
        if let Ok(mut file) = std::fs::OpenOptions::new()
            .create(true)
            .append(true)
            .open(aglet_core::classification::CLASSIFICATION_DEBUG_LOG_PATH)
        {
    ...
    pub(crate) fn move_selected_item_between_slots(
        ...
        let preview =
            Workspace::preview_section_move(&view, from_section.as_ref(), to_section.as_ref());

        match (from_section, to_section) {
            (Some(from_section), Some(to_section)) => {
                workspace.move_item_between_sections(item_id, &view, &from_section, &to_section)?;
            }
            (Some(from_section), None) => {
                workspace.remove_item_from_section(item_id, &view, &from_section)?;
            }
            (None, Some(to_section)) => {
                workspace.insert_item_in_section(item_id, &view, &to_section)?;
            }
            (None, None) => {}
        }
    ...
    fn section_move_status(
        &self,
        target_title: &str,
        preview: &aglet_core::workspace::SectionMovePreview,
    ) -> String {
    ...
    pub(crate) fn insert_into_context(
        ...
        match Self::section_for_context(view, context)? {
            Some(section) => {
                workspace.insert_item_in_section(item_id, view, &section)?;
            }
            None => {
                workspace.insert_item_in_unmatched(item_id, view)?;
            }
        }
        Ok(())
    }
    ...
    pub(crate) fn item_membership_in_view_result(
        item_id: ItemId,
        result: &aglet_core::query::ViewResult,
    ) -> Option<Option<usize>> {
    ...
    pub(crate) fn compute_assignment_preview(&mut self, workspace: &Workspace<'_>) {
        self.item_assign_preview = AssignmentPreview::default();
        ...
        match self.item_assign_pane {
            ...
            ItemAssignPane::ViewSection => {
                ...
                for item_id in &action_ids {
                    ...
                    let preview = aglet_core::workspace::Workspace::preview_section_move(
                        &view,
                        from_section,
                        to_section.as_ref(),
                    );
            ...
            ItemAssignPane::Categories => {
                ...
                for item_id in &action_ids {
                    ...
                    let Ok(hypothetical) =
                        workspace.preview_manual_category_toggle(*item_id, row.id)
                    else {
                        continue;
                    };
    ...
    pub(crate) fn cycle_view(&mut self, delta: i32, workspace: &Workspace<'_>) -> TuiResult<()> {
        if self.views.is_empty() {
    ...
    pub(crate) fn jump_to_all_items_view(&mut self, workspace: &Workspace<'_>) -> TuiResult<()> {
        let Some(index) = self
            .views
            .iter()
