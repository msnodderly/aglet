...
impl App {
    ...
    fn maybe_run_temporal_reevaluation(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        if !aglet.has_date_conditions()? {
            return Ok(());
        }
    ...
    pub(crate) fn maybe_run_auto_refresh(&mut self, aglet: &Aglet<'_>) -> TuiResult<bool> {
        if self.should_auto_refresh_now() {
            self.maybe_run_temporal_reevaluation(aglet)?;
    ...
    pub(crate) fn run(&mut self, terminal: &mut TuiTerminal, aglet: &Aglet<'_>) -> TuiResult<()> {
        self.maybe_run_temporal_reevaluation(aglet)?;
        self.refresh(aglet.store())?;
        self.load_last_view_name(aglet.store())?;
        self.refresh(aglet.store())?; // re-resolve slots for the restored view
        ...
        loop {
            ...
            needs_redraw |= self.process_classification_results(aglet)?;
            if needs_redraw {
            ...
            if !event::poll(std::time::Duration::from_millis(200))? {
                needs_redraw |= self.maybe_run_auto_refresh(aglet)?;
                continue;
            }
            ...
            match event::read()? {
                ...
                Event::Key(key) => {
                    ...
                    let should_quit = match self.handle_key_event(key, aglet) {
                        Ok(value) => value,
                        Err(err) => {
                    ...
                    needs_redraw = true;
                    if should_quit {
                        let _ = self.persist_last_view_name(aglet.store());
                        break;
                    }
    ...
    pub(crate) fn process_classification_results(&mut self, aglet: &Aglet<'_>) -> TuiResult<bool> {
        ...
        while let Some(result) = self.classification.worker.try_recv() {
            ...
            let current_item = match aglet.store().get_item(result.item_id) {
                Ok(item) => item,
            ...
            let queued = aglet.apply_classification_results(
                result.item_id,
    ...
    pub(crate) fn submit_background_classification(
        ...
        if let Some(job) = aglet.prepare_background_classification(item_id, reference_date)? {
            self.classification
                .in_flight_classifications
                .insert(item_id);
            self.debug_log_classification(
    ...
    fn debug_log_classification(&self, aglet: &Aglet<'_>, message: &str) {
        if !aglet.debug_enabled() {
            return;
        }
    ...
    pub(crate) fn move_selected_item_between_slots(
        ...
        let preview =
            Aglet::preview_section_move(&view, from_section.as_ref(), to_section.as_ref());

        match (from_section, to_section) {
            (Some(from_section), Some(to_section)) => {
                aglet.move_item_between_sections(item_id, &view, &from_section, &to_section)?;
            }
            (Some(from_section), None) => {
                aglet.remove_item_from_section(item_id, &view, &from_section)?;
            }
            (None, Some(to_section)) => {
                aglet.insert_item_in_section(item_id, &view, &to_section)?;
            }
            (None, None) => {}
        }
    ...
    fn section_move_status(
        &self,
        target_title: &str,
        preview: &aglet_core::aglet::SectionMovePreview,
    ) -> String {
    ...
    pub(crate) fn insert_into_context(
        ...
        match Self::section_for_context(view, context)? {
            Some(section) => {
                aglet.insert_item_in_section(item_id, view, &section)?;
            }
            None => {
                aglet.insert_item_in_unmatched(item_id, view)?;
            }
        }
        Ok(())
    }
    ...
    pub(crate) fn compute_assignment_preview(&mut self, aglet: &Aglet<'_>) {
        self.item_assign_preview = AssignmentPreview::default();
        ...
        match self.item_assign_pane {
            ...
            ItemAssignPane::ViewSection => {
                ...
                for item_id in &action_ids {
                    ...
                    let preview = aglet_core::aglet::Aglet::preview_section_move(
                        &view,
                        from_section,
                        to_section.as_ref(),
                    );
            ...
            ItemAssignPane::Categories => {
                ...
                for item_id in &action_ids {
                    ...
                    let Ok(hypothetical) = aglet.preview_manual_category_toggle(*item_id, row.id)
                    else {
                        continue;
                    };
    ...
    pub(crate) fn cycle_view(&mut self, delta: i32, aglet: &Aglet<'_>) -> TuiResult<()> {
        if self.views.is_empty() {
    ...
    pub(crate) fn jump_to_all_items_view(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let Some(index) = self
            .views
            .iter()
    ...
    pub(crate) fn begin_global_search_session(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
        let Some(index) = self
            .views
            .iter()
            .position(|view| view.name.eq_ignore_ascii_case("All Items"))
    ...
    pub(crate) fn restore_global_search_session(&mut self, aglet: &Aglet<'_>) -> TuiResult<()> {
