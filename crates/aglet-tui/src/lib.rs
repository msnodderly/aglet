...
use aglet_core::query::{evaluate_query, generate_datebook_sections, resolve_view};
use aglet_core::store::Store;
use aglet_core::workflow::WorkflowConfig;
...
pub fn run_with_options(db_path: &Path, debug: bool) -> TuiResult<()> {
    ...
    let workspace = Workspace::with_debug(&store, &classifier, debug);

    let mut terminal = TerminalSession::enter()?;

    let mut app = App::default();
    let result = app.run(terminal.terminal_mut(), &workspace);
