...
fn run() -> Result<(), String> {
    ...
    temporal_reevaluate_before_command(&workspace)?;
    ...
    match command {
        Command::Add {
            ...
            categories,
            values,
        } => cmd_add(&workspace, text, note, when, categories, values),
        Command::Edit {
            item_id,
            text,
            note,
        ...
        Command::Claim { item_id } => cmd_claim(&workspace, &store, item_id),
        Command::Ready { sort, format } => cmd_ready(&store, sort, format),
        Command::Release { item_id } => cmd_release(&workspace, &store, item_id),
        Command::List {
            view,
            category,
            any_category,
        ...
        Command::Delete { item_id } => cmd_delete(&workspace, item_id),
        Command::Deleted => cmd_deleted(&store),
        Command::Restore { log_id } => cmd_restore(&store, log_id),
        Command::Category { command } => cmd_category(&workspace, &store, command),
        Command::View { command } => cmd_view(&workspace, &store, command),
        Command::Import { command } => cmd_import(&workspace, &store, command),
        Command::Link { command } => cmd_link(&workspace, command),
        Command::Unlink { command } => cmd_unlink(&workspace, command),
        Command::Item(item_cmd) => match item_cmd {
            ...
            ItemCommand::Delete { item_id } => cmd_delete(&workspace, item_id),
        },
        Command::Tui { .. } => Ok(()),
    }
}
...
fn cmd_add(
    ...
    let categories_hierarchy = workspace
        .store()
        .get_hierarchy()
        .map_err(|e| e.to_string())?;
    ...
    for (category_id, category_name) in tag_assignments {
        apply_tag_category_assignment(workspace, item.id, category_id, &category_name)?;
    }
    for assignment in value_assignments {
        apply_numeric_value_assignment(workspace, item.id, assignment)?;
    }
...
fn temporal_reevaluate_before_command(workspace: &Workspace<'_>) -> Result<(), String> {
    if workspace.has_date_conditions().map_err(|e| e.to_string())? {
        let _ = workspace
...
fn cmd_edit(
    ...
    let item_id = resolve_item_id(&item_id_str, workspace.store())?;
...
fn cmd_claim(workspace: &Workspace<'_>, store: &Store, item_id_str: String) -> Result<(), String> {
    let item_id = resolve_item_id(&item_id_str, store)?;
    let workflow = resolved_workflow_or_err(store)?;
...
fn cmd_delete(workspace: &Workspace<'_>, item_id_str: String) -> Result<(), String> {
    let item_id = resolve_item_id(&item_id_str, workspace.store())?;
    workspace
...
fn cmd_link(workspace: &Workspace<'_>, command: LinkCommand) -> Result<(), String> {
    match command {
        LinkCommand::DependsOn {
            item_id,
            depends_on_item_id,
        } => {
            let item_id = resolve_item_id(&item_id, workspace.store())?;
            let depends_on_item_id = resolve_item_id(&depends_on_item_id, workspace.store())?;
            let result = workspace
        ...
        LinkCommand::Blocks {
            ...
            let blocker_item_id = resolve_item_id(&blocker_item_id, workspace.store())?;
            let blocked_item_id = resolve_item_id(&blocked_item_id, workspace.store())?;
            let result = workspace
        ...
        LinkCommand::Related {
            ...
            let item_a_id = resolve_item_id(&item_a_id, workspace.store())?;
            let item_b_id = resolve_item_id(&item_b_id, workspace.store())?;
            let result = workspace
...
fn cmd_unlink(workspace: &Workspace<'_>, command: UnlinkCommand) -> Result<(), String> {
    match command {
        UnlinkCommand::Blocks {
            blocker_item_id,
            blocked_item_id,
        } => unlink_blocks(workspace, blocker_item_id, blocked_item_id),
        UnlinkCommand::DependsOn {
            item_id,
            depends_on_item_id,
        } => unlink_depends_on(workspace, item_id, depends_on_item_id),
        UnlinkCommand::Related {
            item_a_id,
            item_b_id,
        } => unlink_related(workspace, item_a_id, item_b_id),
    }
}
...
fn cmd_category(
    ...
    match command {
        ...
        CategoryCommand::Create {
            ...
            let result = match workspace.create_category(&category) {
                Ok(result) => result,
        ...
        CategoryCommand::AddAction {
            ...
            let (action_index, result) = workspace
...
fn cmd_view(workspace: &Workspace<'_>, store: &Store, command: ViewCommand) -> Result<(), String> {
    let _ = workspace;
    match command {
        ViewCommand::List => {
...
fn resolve_db_path(db_opt: Option<PathBuf>) -> Result<PathBuf, String> {
    let path = if let Some(path) = db_opt {
        ...
        PathBuf::from(home).join(".workspace").join("default.ag")
    };
...
mod tests {
    ...
    fn clap_parses_claim_with_item_id() {
        let cli =
            Cli::try_parse_from(["workspace", "claim", "123e4567-e89b-12d3-a456-426614174000"])
                .expect("parse CLI");
    ...
    fn clap_parses_ready_command() {
        let cli = Cli::try_parse_from(["workspace", "ready", "--sort", "item", "--format", "json"])
            .expect("parse CLI");
    ...
    fn cmd_claim_fails_when_item_is_already_claimed() {
        ...
        let err =
            cmd_claim(&workspace, &store, item.id.to_string()).expect_err("claim should fail");
    ...
    fn cmd_claim_assigns_claim_category_and_keeps_ready_category() {
        ...
        cmd_claim(&workspace, &store, item.id.to_string()).expect("claim should succeed");
    ...
    fn cmd_release_removes_claim_category() {
        ...
        workspace
            .assign_item_manual(item.id, ready.id, Some("manual:test.assign".to_string()))
            .expect("seed ready");
        workspace.claim_item_workflow(item.id).expect("claim");

        cmd_release(&workspace, &store, item.id.to_string()).expect("release should succeed");
    ...
    fn clap_parses_top_level_unlink_depends_on_subcommand() {
        let cli = Cli::try_parse_from(["workspace", "unlink", "depends-on", "a", "b"])
            .expect("parse CLI");
    ...
    fn clap_parses_list_with_blocked_flag() {
        let cli = Cli::try_parse_from(["workspace", "list", "--blocked"]).expect("parse CLI");
    ...
    fn clap_parses_list_with_not_blocked_flag() {
        let cli = Cli::try_parse_from(["workspace", "list", "--not-blocked"]).expect("parse CLI");
    ...
    fn clap_parses_view_show_with_sort_flag() {
        let cli = Cli::try_parse_from(["workspace", "view", "show", "All Items", "--sort", "when"])
            .expect("parse CLI");
    ...
    fn clap_parses_list_with_json_format() {
        let cli =
            Cli::try_parse_from(["workspace", "list", "--format", "json"]).expect("parse CLI");
    ...
    fn clap_parses_search_with_json_format() {
        let cli = Cli::try_parse_from(["workspace", "search", "foo", "--format", "json"])
            .expect("parse CLI");
    ...
    fn clap_parses_search_with_blocked_flag() {
        let cli = Cli::try_parse_from(["workspace", "search", "foo", "--blocked"]).expect("parse");
    ...
    fn clap_parses_view_show_with_blocked_flag() {
        let cli = Cli::try_parse_from(["workspace", "view", "show", "All Items", "--blocked"])
            .expect("parse");
    ...
    fn clap_parses_view_show_with_json_format() {
        let cli =
            Cli::try_parse_from(["workspace", "view", "show", "All Items", "--format", "json"])
                .expect("parse CLI");
    ...
    fn clap_parses_view_clone_command() {
        let cli = Cli::try_parse_from(["workspace", "view", "clone", "Source", "Target"])
            .expect("parse CLI");
    ...
    fn cmd_add_rejects_empty_text() {
        ...
        let err = cmd_add(&workspace, "".to_string(), None, None, vec![], vec![])
    ...
    fn cmd_claim_fails_when_no_workflow_configured() {
        ...
        let err = cmd_claim(&workspace, &store, item.id.to_string())
