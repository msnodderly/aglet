...
use super::{
    ...
    ViewAppearanceRow, ViewAssignRow, ViewEditPaneFocus, ViewEditRegion, ViewEditTab, ViewScopeRow,
    ViewSectionsSettingsRow, WorkflowRolePickerOrigin,
};
...
use agenda_core::model::{
    ...
    CriterionMode, DatebookConfig, DatebookInterval, DatebookPeriod, EmptySections, Item, ItemId,
...
fn category_direct_edit_save_applies_mixed_diff_and_preserves_non_column_assignments() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-direct-edit-save-diff-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn edit_panel_prechecks_derived_assignments() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-edit-derived-checks-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn view_picker_enter_switches_to_ready_queue_and_filters_blocked_items() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-ready-queue-switch-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn view_picker_lowercase_n_opens_view_create() {
    ...
    // n opens ViewEdit directly at the Scope view-type row.
    assert_eq!(app.mode, Mode::ViewEdit);
    ...
    assert_eq!(state.scope_row, ViewScopeRow::ViewType);
    assert_eq!(state.inline_input, None);
...
fn view_create_name_save_opens_unsaved_view_wizard_with_first_section_editing() {
    ...
    // N now opens ViewEdit directly at the Scope view-type row.
    app.handle_view_picker_key(KeyCode::Char('n'), &agenda)
    ...
    // Move to Name and type the view name.
...
fn hide_dependent_session_toggle_resets_on_view_switch() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-hide-dependent-reset-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn item_assign_input_enter_autoselects_single_visible_match() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-assign-single-match-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn item_assign_picker_filter_limits_rows_and_enter_uses_exact_match() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-assign-picker-filter-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn item_assign_picker_enter_creates_category_from_filter_text() {
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!(
        "agenda-tui-assign-picker-create-from-filter-{nanos}.ag"
    ));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn assign_picker_render_updated_footer_copy_and_hides_reserved_done() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-assign-copy-render-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn assign_picker_shows_numeric_category_as_toggle_not_inline_value() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-assign-numeric-row-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn assign_picker_hides_system_views_from_assignment_targets() {
    ...
    assert!(
        !labels
            .iter()
            .any(|label| label == agenda_core::workflow::READY_QUEUE_VIEW_NAME),
...
fn category_manager_enter_focuses_details_instead_of_opening_config_editor() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-category-config-open-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn category_manager_filter_focus_types_text_instead_of_triggering_commands() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-category-filter-focus-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn category_manager_slash_arms_filter_without_inserting_slash() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-category-filter-slash-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn category_manager_tab_focuses_details_and_p_no_longer_opens_reparent_ui() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-category-filter-tab-p-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn category_manager_upper_j_reorders_selected_category_down_among_siblings() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-category-reorder-down-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn category_manager_upper_h_on_root_is_noop_with_status() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-category-outdent-root-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn select_workflow_picker_category_for_test(app: &mut App, agenda: &Agenda<'_>, target_name: &str) {
...
fn category_manager_numeric_initial_focus_is_integer_toggle() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-catmgr-numeric-focus-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn category_manager_numeric_integer_toggle_restores_two_decimal_default() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-catmgr-format-cycle-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn category_manager_numeric_fields_edit_and_persist() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-catmgr-format-fields-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn category_manager_format_cycle_does_not_reclassify_items() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-catmgr-format-stable-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn batch_item_assign_hidden_reserved_done_keeps_picker_and_selection() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-batch-d-picker-esc-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn batch_item_assign_hidden_reserved_done_does_not_mark_selected_items_done() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-batch-d-picker-apply-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn make_test_store_with_view(suffix: &str) -> (Store, std::path::PathBuf) {
    let nanos = SystemTime::now()
        ...
        .expect("system clock should be after epoch")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-view-edit-{suffix}-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    store
...
fn view_edit_tab_cycles_panes() {
    ...
    app.handle_view_edit_key(KeyCode::Char('2'), &agenda)
        .expect("jump to sections tab");
    assert_eq!(
        app.view_edit_state.as_ref().unwrap().pane_focus,
        ViewEditPaneFocus::Sections
    );
    app.handle_view_edit_key(KeyCode::Tab, &agenda)
        .expect("sections -> details");
    assert_eq!(
        app.view_edit_state.as_ref().unwrap().pane_focus,
        ViewEditPaneFocus::Details
    );
...
fn view_edit_details_jk_moves_between_criteria_and_view_aux_rows() {
    ...
    app.handle_view_edit_key(KeyCode::Char('j'), &agenda)
        .expect("when exclude -> hide dependent");
    assert_eq!(
        app.view_edit_state.as_ref().unwrap().scope_row,
    ...
    app.handle_view_edit_key(KeyCode::Char('k'), &agenda)
        .expect("hide dependent -> when exclude");
    assert_eq!(
        app.view_edit_state.as_ref().unwrap().scope_row,
        ViewScopeRow::DateExclude
    );
...
#[test]
fn view_edit_scope_datebook_config_stays_next_to_view_type() {
    let (store, db_path) = make_test_store_with_view("scope-datebook-config");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
    ...
    app.handle_view_edit_key(KeyCode::Char(' '), &agenda)
        .expect("toggle datebook");
    let state = app.view_edit_state.as_ref().expect("view edit state");
    let config = state
        .draft
        .datebook_config
        .as_ref()
        .expect("datebook config should be created inline");
    assert_eq!(config.period, DatebookPeriod::Week);
    assert_eq!(config.interval, DatebookInterval::Daily);
    assert_eq!(state.scope_row, ViewScopeRow::ViewType);
    ...
    let rendered = terminal_buffer_lines(&terminal).join("\n");
    assert!(
        rendered.contains("View type") && rendered.contains("datebook"),
        "Scope should show the Datebook view type: {rendered}"
    );
    assert!(
        rendered.contains("Period")
            && rendered.contains("Interval")
            && rendered.contains("Anchor")
            && rendered.contains("Date source"),
        "Datebook config should render inline under View type: {rendered}"
    );

    app.handle_view_edit_key(KeyCode::Char('j'), &agenda)
        .expect("view type -> period");
    assert_eq!(
        app.view_edit_state.as_ref().unwrap().scope_row,
...
#[test]
fn view_edit_datebook_sections_tab_wayfinding_returns_to_scope() {
    let (store, db_path) = make_test_store_with_view("datebook-sections-wayfinding");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
    ...
    assert_eq!(
        app.view_edit_state.as_ref().unwrap().sections_settings_row,
    ...
    let rendered = terminal_buffer_lines(&terminal).join("\n");
    assert!(
        rendered.contains("Sections generated from Scope datebook settings")
            && rendered.contains("Current preview")
            && rendered.contains("Enter returns to Scope"),
        "Datebook Sections tab should guide users back to Scope: {rendered}"
    );

    app.handle_view_edit_key(KeyCode::Enter, &agenda)
        .expect("open datebook sections details");
    assert_eq!(
        app.view_edit_state.as_ref().unwrap().pane_focus,
        ViewEditPaneFocus::Details
    );
    app.handle_view_edit_key(KeyCode::Enter, &agenda)
        .expect("return to scope");
    let state = app.view_edit_state.as_ref().expect("view edit state");
    assert_eq!(state.active_tab, ViewEditTab::Scope);
    assert_eq!(state.scope_row, ViewScopeRow::ViewType);
    assert!(app.status.contains("generated from Scope"));
...
fn view_edit_unmatched_enter_uses_selected_details_row() {
    ...
    app.handle_view_edit_key(KeyCode::Char('2'), &agenda)
        .expect("jump to sections");
    app.handle_view_edit_key(KeyCode::Enter, &agenda)
        .expect("open view settings details");
    assert_eq!(
        app.view_edit_state.as_ref().unwrap().sections_settings_row,
...
fn view_edit_sections_can_select_view_properties_row_and_enter_opens_criteria() {
    ...
    app.handle_view_edit_key(KeyCode::Enter, &agenda)
        .expect("enter should open view settings details");
    assert_eq!(
        app.view_edit_state.as_ref().unwrap().region,
        ViewEditRegion::Sections
    );
...
fn view_edit_p_toggles_preview_and_tab_cycles_preview_pane() {
    ...
    // Tab moves between the local Details and Preview panes on non-Sections tabs.
    app.handle_view_edit_key(KeyCode::Tab, &agenda)
...
fn view_edit_save_persists_view() {
    ...
    // Toggle view default display mode in Appearance.
    app.handle_view_edit_key(KeyCode::Char('3'), &agenda)
        .expect("jump to appearance");
    app.handle_view_edit_key(KeyCode::Char('m'), &agenda)
        .expect("toggle view display mode");
...
fn horizontal_singleline_cards_preserve_glyphs() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-horizontal-singleline-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn board_sorting_angle_brackets_alias_ascending_and_descending_sort() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-sort-angle-aliases-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
    let classifier = SubstringClassifier;
    let agenda = Agenda::new(&store, &classifier);
...
fn board_render_summary_row_supports_all_summary_functions() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-summary-row-all-fns-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
...
fn shift_f_on_non_numeric_column_is_noop() {
    let nanos = SystemTime::now()
        ...
        .expect("system clock")
        .as_nanos();
    let db_path = std::env::temp_dir().join(format!("agenda-tui-shift-f-nonnumeric-{nanos}.ag"));
    let store = Store::open(&db_path).expect("open temp db");
