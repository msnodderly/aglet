    ...
    fn suggestion_review_space_toggles_and_esc_cancels() {
        ...
        assert!(!app.classification.suggestion_review.as_ref().unwrap().items[0].suggestions[0].accepted);
    ...
    fn ollama_model_picker_selects_and_persists_model() {
        ...
        app.handle_key(KeyCode::Char('j'), &agenda)
            .expect("picker down");
        assert_eq!(
            app.settings.ollama_model_picker.as_ref().map(|p| p.selected_index),
            Some(2)
        );
    ...
    fn global_settings_workflow_picker_opens_and_closes() {
        ...
        app.handle_key(KeyCode::Enter, &agenda)
            .expect("open claim result picker");
        assert_eq!(
            app.settings.workflow_role_picker
                .as_ref()
                .expect("workflow role picker should be open")
                .row_index,
            0
