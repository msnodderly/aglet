...
/// as computed by [`Workspace::preview_section_move`].
...
impl<'a> Workspace<'a> {
    ...
    pub fn with_debug(store: &'a Store, classifier: &'a dyn Classifier, debug: bool) -> Self {
        let mut workspace = Self::new(store, classifier);
    ...
    pub fn preview_manual_category_toggle(
        ...
        let preview_agenda = Workspace::new(&preview_store, self.classifier);
    ...
    fn debug_log_process_result(&self, context: &str, item_id: ItemId, result: &ProcessItemResult) {
        ...
        for event in &result.assignment_events {
            eprintln!(
                "[workspace.debug] context={context} item={item_id} event={:?} category={} summary={}",
                event.kind, event.category_name, event.summary
            );
        }
        for message in &result.semantic_debug_messages {
            eprintln!("[workspace.debug] context={context} item={item_id} semantic={message}");
        }
    }
    ...
    fn debug_log_link_event(
        ...
        eprintln!(
            "[workspace.debug] event={family} kind={kind:?} item={item_id} other={other_item_id}"
        );
    }
...
mod tests {
    ...
    fn create_item_triggers_classification() {
        ...
        let result = workspace.create_item(&item).unwrap();
    ...
    fn create_item_does_not_match_literal_category_name_when_disabled() {
        ...
        let person_result = workspace.create_item(&person_item).unwrap();
        assert!(!person_result.new_assignments.contains(&person.id));

        let bob_item = Item::new("Call Bob tomorrow".to_string());
        let bob_result = workspace.create_item(&bob_item).unwrap();
    ...
    fn create_item_with_classification_disabled_skips_implicit_matching() {
        ...
        workspace.create_item(&item).unwrap();
    ...
    fn literal_auto_apply_and_semantic_review_can_run_together() {
        ...
        let workspace = Workspace::with_ollama_transport(&store, &classifier, transport);
    ...
    fn semantic_review_keeps_highest_confidence_suggestion_per_exclusive_family() {
        ...
        workspace.create_category(&critical).unwrap();
        let mut high = child_category("High", priority.id, false);
        high.enable_semantic_classification = true;
        workspace.create_category(&high).unwrap();
        let mut normal = child_category("Normal", priority.id, false);
        ...
        workspace.create_category(&low).unwrap();
    ...
    fn update_item_keeps_manual_exclusive_choice_over_later_auto_classified_action() {
        ...
        workspace.update_item(&updated).unwrap();
    ...
    fn create_item_unknown_hashtag_does_not_auto_create_category() {
        ...
        let _ = workspace.create_item(&item).unwrap();
    ...
    fn update_item_triggers_reclassification() {
        ...
        let result = workspace.update_item(&updated).unwrap();
    ...
    fn create_category_triggers_retroactive_classification() {
        ...
        let sarah = category("Sarah", true);
        let result = workspace.create_category(&sarah).unwrap();
    ...
    fn update_category_triggers_reclassification() {
        ...
        workspace.create_category(&foo).unwrap();

        let existing = Item::new("meeting with Foo".to_string());
        workspace.create_item(&existing).unwrap();
        assert!(store
        ...
        let update_result = workspace.update_category(&renamed).unwrap();
        ...
        workspace.create_item(&new_item).unwrap();
    ...
    fn add_category_action_does_not_retroactively_fire_for_existing_assignments() {
        ...
        workspace.create_category(&source).unwrap();
        workspace.create_category(&notify).unwrap();

        let item = Item::new("Task".to_string());
        workspace.create_item(&item).unwrap();
        workspace
            .assign_item_manual(item.id, source.id, None)
            .unwrap();

        let (_index, result) = workspace
            .add_category_action(
                source.id,
                Action::Assign {
    ...
    fn manual_unassign_blocks_removing_ancestor_while_descendant_assigned() {
        ...
        let err = workspace
            .unassign_item_manual(item.id, work.id)
            .unwrap_err();
        assert!(matches!(err, AgletError::InvalidOperation { .. }));
    ...
    fn manual_unassign_removes_live_subsumption_ancestor() {
        ...
        workspace
            .unassign_item_manual(item.id, project_y.id)
            .unwrap();
    ...
    fn manual_unassign_allows_removing_leaf_then_parent() {
        ...
        workspace
            .unassign_item_manual(item.id, project_y.id)
            .unwrap();
        workspace.unassign_item_manual(item.id, work.id).unwrap();
    ...
    fn claim_item_manual_race_allows_only_one_winner() {
        let nanos = SystemTime::now()
            ...
            .expect("clock")
            .as_nanos();
        let db_path = std::env::temp_dir().join(format!("workspace-claim-race-{nanos}.ag"));
    ...
    fn manual_assignment_rejects_duplicate_category_names() {
        ...
        workspace.create_category(&work_priority).unwrap();

        let mut project_priority = child_category("Priority", project_x.id, false);
        project_priority.is_exclusive = true;
        let err = workspace.create_category(&project_priority).unwrap_err();
    ...
    fn engine_error_does_not_prevent_store_mutation() {
        ...
        let err = workspace.create_item(&item).unwrap_err();
        match err {
    ...
    fn db_backed_setup_with_items_categories_views_and_assignments_resolves_filters() {
        ...
        workspace.create_item(&collaborative).unwrap();
        workspace
        ...
        workspace.create_item(&solo).unwrap();
        workspace
    ...
    fn mark_item_done_sets_done_fields_and_assigns_done_category() {
        ...
        let _result = workspace.mark_item_done(item.id).unwrap();
        let loaded = store.get_item(item.id).unwrap();
    ...
    fn claim_item_workflow_assigns_claim_target_for_ready_item() {
        ...
        workspace.claim_item_workflow(item.id).unwrap();
    ...
    fn mark_item_done_clears_workflow_claim_assignment() {
        ...
        workspace.mark_item_done(item.id).unwrap();
    ...
    fn mark_item_done_rejects_non_actionable_only_items() {
        ...
        workspace.create_category(&reference).unwrap();
        ...
        let err = workspace.mark_item_done(item.id).unwrap_err();
        assert!(matches!(err, AgletError::InvalidOperation { .. }));
    }
    ...
    fn toggle_item_done_unsets_done_state_and_done_assignment() {
        ...
        workspace.toggle_item_done(item.id).unwrap();
    ...
    fn move_category_to_parent_reparents_category() {
        ...
        workspace.create_category(&left).unwrap();
        workspace.create_category(&right).unwrap();

        let child = child_category("Child", left.id, false);
        workspace.create_category(&child).unwrap();
    ...
    fn move_category_within_parent_reorders_children() {
        ...
        workspace.create_category(&parent).unwrap();
        let alpha = child_category("Alpha", parent.id, false);
        let beta = child_category("Beta", parent.id, false);
        workspace.create_category(&alpha).unwrap();
        workspace.create_category(&beta).unwrap();

        workspace.move_category_within_parent(beta.id, -1).unwrap();
    ...
    fn link_items_depends_on_rejects_self_link() {
        ...
        let err = workspace
            .link_items_depends_on(item_id, item_id)
            .unwrap_err();
        assert!(matches!(err, AgletError::InvalidOperation { .. }));
    }
    ...
    fn link_items_related_rejects_self_link() {
        ...
        let err = workspace.link_items_related(item_id, item_id).unwrap_err();
        assert!(matches!(err, AgletError::InvalidOperation { .. }));
    }
    ...
    fn link_items_blocks_stores_inverse_depends_on_edge() {
        ...
        let result = workspace.link_items_blocks(blocker, blocked).unwrap();
        assert!(result.created);
        assert!(store
            .item_link_exists(blocked, blocker, ItemLinkKind::DependsOn)
            .unwrap());
        assert_eq!(
            workspace.immediate_dependent_ids(blocker).unwrap(),
            vec![blocked]
        );
        assert_eq!(
            workspace.immediate_prereq_ids(blocked).unwrap(),
            vec![blocker]
        );
    }
    ...
    fn link_items_related_normalizes_pair_and_is_idempotent() {
        ...
        let first = workspace.link_items_related(high, low).unwrap();
        let second = workspace.link_items_related(low, high).unwrap();
    ...
    fn link_items_depends_on_rejects_direct_cycle() {
        ...
        workspace.link_items_depends_on(a, b).unwrap();
        let err = workspace.link_items_depends_on(b, a).unwrap_err();
        assert!(matches!(err, AgletError::InvalidOperation { .. }));
    }
    ...
    fn link_items_depends_on_rejects_longer_cycle() {
        ...
        workspace.link_items_depends_on(a, b).unwrap();
        workspace.link_items_depends_on(b, c).unwrap();
        let err = workspace.link_items_depends_on(c, a).unwrap_err();
        assert!(matches!(err, AgletError::InvalidOperation { .. }));
    }
    ...
    fn link_items_blocks_rejects_direct_cycle() {
        ...
        workspace.link_items_blocks(a, b).unwrap();
        let err = workspace.link_items_blocks(b, a).unwrap_err();
        assert!(matches!(err, AgletError::InvalidOperation { .. }));
    }
    ...
    fn link_items_blocks_rejects_longer_cycle() {
        ...
        workspace.link_items_blocks(a, b).unwrap();
        workspace.link_items_blocks(b, c).unwrap();
        let err = workspace.link_items_blocks(c, a).unwrap_err();
        assert!(matches!(err, AgletError::InvalidOperation { .. }));
    }
    ...
    fn link_items_related_allows_triangle() {
        ...
        assert!(workspace.link_items_related(a, b).unwrap().created);
        assert!(workspace.link_items_related(b, c).unwrap().created);
        assert!(workspace.link_items_related(c, a).unwrap().created);

        let links_a = workspace.immediate_related_ids(a).unwrap();
        let links_b = workspace.immediate_related_ids(b).unwrap();
        let links_c = workspace.immediate_related_ids(c).unwrap();
    ...
    fn unlink_items_blocks_and_related_are_idempotent() {
        ...
        workspace.link_items_blocks(a, b).unwrap();
        workspace.link_items_related(a, b).unwrap();

        workspace.unlink_items_blocks(a, b).unwrap();
        workspace.unlink_items_related(a, b).unwrap();
        // idempotent delete behavior delegated to Store
        workspace.unlink_items_blocks(a, b).unwrap();
        workspace.unlink_items_related(a, b).unwrap();

        assert!(workspace.immediate_dependent_ids(a).unwrap().is_empty());
        assert!(workspace.immediate_prereq_ids(b).unwrap().is_empty());
        assert!(workspace.immediate_related_ids(a).unwrap().is_empty());
        assert!(workspace.immediate_related_ids(b).unwrap().is_empty());
    }
    ...
    fn immediate_links_for_item_groups_prereqs_blocks_and_related() {
        ...
        workspace.link_items_depends_on(a, b).unwrap();
        workspace.link_items_blocks(a, c).unwrap(); // c depends-on a
        workspace.link_items_related(a, d).unwrap();

        let links = workspace.immediate_links_for_item(a).unwrap();
    ...
    fn normalize_related_pair_returns_lexicographic_order() {
        ...
        let (lo, hi) = Workspace::normalize_related_pair(a, b);
        assert_eq!(lo, a);
        assert_eq!(hi, b);

        // Reversed input should also produce (a, b).
        let (lo2, hi2) = Workspace::normalize_related_pair(b, a);
        assert_eq!(lo2, a);
        assert_eq!(hi2, b);
    }
    ...
    fn normalize_related_pair_is_idempotent() {
        ...
        let first = Workspace::normalize_related_pair(a, b);
        let second = Workspace::normalize_related_pair(first.0, first.1);
    ...
    fn ensure_not_self_link_rejects_identical_ids() {
        ...
        let result = workspace.ensure_not_self_link(id, id, "depends-on");
    ...
    fn ensure_not_self_link_accepts_distinct_ids() {
        ...
        assert!(
            workspace.ensure_not_self_link(a, b, "depends-on").is_ok(),
    ...
    fn ensure_depends_on_no_cycle_detects_direct_cycle() {
        ...
        let result = workspace.ensure_depends_on_no_cycle(b, a);
    ...
    fn ensure_depends_on_no_cycle_detects_transitive_cycle() {
        ...
        let result = workspace.ensure_depends_on_no_cycle(c, a);
        assert!(
            result.is_err(),
    ...
    fn ensure_depends_on_no_cycle_allows_non_cyclic_dependency() {
        ...
        assert!(
            workspace.ensure_depends_on_no_cycle(c, a).is_ok(),
    ...
    fn preview_section_move_none_to_none_is_empty() {
        let v = view("Empty");
        let preview = Workspace::preview_section_move(&v, None, None);
    ...
    fn preview_section_move_none_to_section_assigns_insert_targets() {
        ...
        let preview = Workspace::preview_section_move(&v, None, Some(&sec_a));
    ...
    fn preview_section_move_section_to_none_unassigns_remove_targets() {
        ...
        let preview = Workspace::preview_section_move(&v, Some(&sec_a), None);
    ...
    fn preview_section_move_between_sections_net_change() {
        ...
        let preview = Workspace::preview_section_move(&v, Some(&sec_a), Some(&sec_b));
    ...
    fn preview_section_move_same_section_to_unassign_is_empty() {
        ...
        let preview = Workspace::preview_section_move(&v, Some(&sec_a), Some(&sec_a));
        assert!(
    ...
    fn preview_section_move_on_remove_unassign_included() {
        ...
        let preview = Workspace::preview_section_move(&v, Some(&sec), None);
    ...
    fn mark_recurring_item_done_generates_successor() {
        ...
        let result = workspace.mark_item_done(item.id).unwrap();
    ...
    fn successor_copies_sticky_manual_assignments_not_reserved() {
        ...
        let project = category("Project", false);
        workspace.create_category(&project).unwrap();
    ...
    fn repeated_completion_creates_chain() {
        ...
        let result_a = workspace.mark_item_done(item.id).unwrap();
        ...
        let result_b = workspace.mark_item_done(b_id).unwrap();
